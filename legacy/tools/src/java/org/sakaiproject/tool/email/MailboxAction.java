...
//			}
	...
	*/
										...
										SessionState state)
	{
		...
		if (helperMode != null)
		{
			...
			if (template == null)
			{
				addAlert(state, rb.getString("theisapro"));
			}
			else
			{
				return template;
	...
	*/
	 ...
	 */
	 /*
	private MailArchiveMessage adjustPaging(Context context, SessionState state)
	{
		MailArchiveMessage message = null;
		List messages = (List) state.getAttribute(STATE_ALL_MESSAGES);
		...
		if (pos > 0)
		{
			context.put("goPPButton", Boolean.TRUE);
		}
		else
		{
			context.put("goPPButton", Boolean.FALSE);
		}
		if (pos < sizeResources(state)-1)
		{
			context.put("goNPButton", Boolean.TRUE);
		}
		else
		{
			context.put("goNPButton", Boolean.FALSE);
		}
										...
										SessionState state)
	{
		...
		try
		{
			MailArchiveChannel channel = MailArchiveService.getMailArchiveChannel(
					(String) state.getAttribute(STATE_CHANNEL_REF));
			message =
				channel.getMailArchiveMessage((String) state.getAttribute(STATE_MSG_VIEW_ID));
			context.put("email", message);
		}
		catch (IdUnusedException e) {}
		catch (PermissionException e) {}
	...
	public void doView(RunData runData, Context context)
	{
		...
		if (resources != null)
		{
			for (int i = 0; i < resources.size(); i++)
			{
				...
				if (((MailArchiveMessage) (resources.get(i))).getId().equals(id))
					pos = i;
			}
		}
...
//				}
	...
	*/
	public void doPermissions(RunData data, Context context)
	{
		...
		state.setAttribute(PermissionsAction.STATE_DESCRIPTION,rb.getString("setperm")
				+ SiteService.getSiteDisplay(channelRef.getContext()));

		// ... showing only locks that are prpefixed with this
		state.setAttribute(PermissionsAction.STATE_PREFIX, rb.getString("mailpoi"));
