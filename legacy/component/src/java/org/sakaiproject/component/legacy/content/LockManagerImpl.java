...
import org.sakaiproject.service.legacy.content.Lock;
...
public class LockManagerImpl extends HibernateDaoSupport implements LockManager {
     ...
     * (non-Javadoc)
     * 
     * @see org.sakaiproject.component.legacy.content.LockManagerIntf#lockObject(java.lang.String,
     *      java.lang.String, java.lang.String, boolean)
     */
    public void lockObject(String assetId, String qualifierId, String reason,
            boolean system) {
     ...
     * (non-Javadoc)
     * 
     * @see org.sakaiproject.component.legacy.content.LockManagerIntf#removeLock(java.lang.String,
     *      java.lang.String)
     */
    ...
    protected Lock findLock(String assetId, String qualifierId) {
        try {
            ...
            return (Lock) safePopList(getHibernateTemplate().findByNamedQuery(
                    "getLock", new Object[] { assetId, qualifierId }));
        } catch (HibernateSystemException e) {
            logger.debug("lock with assetId=" + assetId + " and qualifierId= "
                    + qualifierId + "not found: " + e.getMessage());
            return null;
    ...
    protected Lock findOrCreateLock(String assetId, String qualifierId,
            boolean expected) {
        Lock lock = findLock(assetId, qualifierId);
        if (lock == null) {
            if (expected == true) {
                logger.warn("expected Lock not found: " + assetId + ", "
                        + qualifierId);
            }
            return new Lock();
        }
        ...
        if (expected == false) {
            logger.warn("Lock not expected, but found anyway: " + assetId
                    + ", " + qualifierId);
        }
        return lock;
    ...
    public Collection getLocks(String assetId) {
        ...
        try {
            getHibernateTemplate().setCacheQueries(true);
            locks = getHibernateTemplate().findByNamedQuery("activeByAsset",
                    assetId);
        } catch (HibernateObjectRetrievalFailureException e) {
            logger.error("", e);
