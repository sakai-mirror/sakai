    ...
    <xsl:variable name="questionLayout" select="/stxx/questestinterop/assessment/qtimetadata/qtimetadatafield/fieldlabel[text()='QUESTION_LAYOUT']/following-sibling::fieldentry"/>
     ...
     <!--******************************************************************-->
      ...
      <tr>
        ...
        <td>
           <input disabled = "true" name="{$entryXpath}" type="radio" value="I">
            <xsl:if test="$questionLayout ='I'">
              <xsl:attribute name="checked">true</xsl:attribute>
            </xsl:if>
          </input> Each question is on a separate Web page.<br/>
         
          <input disabled = "true" name="{$entryXpath}" type="radio" value="S">
            <xsl:if test="$questionLayout ='S'">
              <xsl:attribute name="checked">true</xsl:attribute>
            </xsl:if>
          </input> Each Part is on a separate Web page. <br/> 
         
          <input disabled = "true" name="{$entryXpath}" type="radio" value="A">
      ...
      <tr>
        ...
        <td>
           <input disabled = "true" name="{$navXpath}" id="{$navXpath}" type="radio" value="RANDOM">
          ...
          <input disabled = "true" name="{$navXpath}" id="{$navXpath}" type="radio" value="LINEAR">
      ...
      <tr>
        ...
        <td>
          <input disabled = "true" name="{$num_entryXpath}" id="{$num_entryXpath}1" type="radio" value="CONTINUOUS">
            <xsl:if test="$questionNumbering ='CONTINUOUS'">
              <xsl:attribute name="checked">true</xsl:attribute>
            </xsl:if>
          </input> Continuous numbering between part.<br/>
         
          <input disabled = "true" name="{$num_entryXpath}" id="{$num_entryXpath}1" type="radio" value="RESTART">
