 ...
 var divs = document.getElementsByTagName("A");
  for (var i = 0; i < divs.length; i++) {
     ...
     if (d.className == "folder"){
        d.style.backgroundImage = "url(<%=request.getContextPath()%>/jsp/aam/images/folder-open.gif)";
     }
   }
  }
...
function toggleRows(elm) {
 ...
 elm.style.backgroundImage = "url(<%=request.getContextPath()%>/jsp/aam/images/folder-closed.gif)";
 var newDisplay = "none";
  ...
  for (var i = 0; i < rows.length; i++) {
   ...
   if (matchStart(r.id, thisID, true)) {
    if (r.style.display == "none") {
     ...
     elm.style.backgroundImage = "url(<%=request.getContextPath()%>/jsp/aam/images/folder-open.gif)";
    }
    break;
   }
 }
 ...
 for (var j = 0; j < rows.length; j++) {
   ...
   if (matchStart(s.id, thisID, matchDirectChildrenOnly)) {
     ...
     if (folder.getAttribute("onclick") != null) {
      folder.style.backgroundImage = "url(<%=request.getContextPath()%>/jsp/aam/images/folder-closed.gif)";
     }
   }
 }
}
