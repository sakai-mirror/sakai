 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class SelectAssessmentResultAction
  extends Action
{
  private static final String RESPROCESSING_XSL_PATH =
    "/xml/xsl/result/resprocessing.xsl";
  private static org.apache.log4j.Logger LOG =
    org.apache.log4j.Logger.getLogger(SelectAssessmentResultAction.class);
    ...
    throws DigitalRepositoryException, SharedException
  {
    ...
    Document quesTestInterop =
      (new XmlDeliveryService()).getAssessmentQTIInterop(assessmentId);
    Document resultReport = ResultReporter.getAssessmentResult(assessmentId);
    ResultProcessor resultsProcessor = new ResultProcessor(this.getServlet().getServletContext(), quesTestInterop);
    Document gradedReport = resultsProcessor.processResults(resultReport);
       
    this.saveDocument(request, resultsProcessor.getXsl());
    this.saveDocument(request, quesTestInterop);
    
    String forward = request.getParameter("forward");
    if (forward.equals("viewResult"))
    {
      this.saveDocument(request, resultReport);
    }
    else
    {
      this.saveDocument(request, resultReport);
      request.getSession().setAttribute("QTI_RESULT_REPORT", resultReport);
    }
