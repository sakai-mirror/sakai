...
public class ItemResultWriter
{
   ...
   */
  public ItemResultWriter(String assessmentId)
  {
    if(LOG.isDebugEnabled())
    {
      LOG.debug("ItemResultWriter(String " + assessmentId + ")");
    }
  ...
  public boolean isEmptyResponseValue()
  {
    if(LOG.isDebugEnabled())
    {
      LOG.debug("isEmptyResponseValue()");
    }
  ...
  public void updateItemResult(HttpServletRequest request)
  {
    if(LOG.isDebugEnabled())
    {
      LOG.debug("updateItemResult(HttpServletRequest " + request + ")");
    }

    createItemResult(request);
    increaseNumAttempts();
    if("grp".equals(request.getParameter("response_type")))
    {
      processGRP(request);
    }
  }
  ...
  private void createItemResult(HttpServletRequest request)
  {
    if(LOG.isDebugEnabled())
    {
      LOG.debug("createItemResult(HttpServletRequest " + request + ")");
    }

    String itemIdent = (String) request.getParameter("item_ident");
    ...
    try
    {
      while(enumer.hasMoreElements())
      {
        ...
        if(
          ! name.equals("action") && ! name.equals("markForReview") &&
            ! name.equals("response_type") && ! name.equals("item_ident"))
        {
          ...
          Element responseValue =
            this.itemResultDoc.createElement(
              QTIConstantStrings.RESPONSE_VALUE);
          Text responseValueText = this.itemResultDoc.createTextNode(value);
