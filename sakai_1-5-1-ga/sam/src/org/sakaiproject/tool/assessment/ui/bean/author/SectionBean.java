...
private String destSectionId; //destinated section where questions will be moved to
 ...
 /**
  * Get ArrayList of SelectItems for available pools
  * @return
  */
  public ArrayList getPoolSelectItems()
  {
    ArrayList list = new ArrayList();

    if (poolsAvailable == null) return list;

    for (int i = 0; i < poolsAvailable.size(); i++) {
      SelectItem selection = new SelectItem();
      QuestionPoolFacade qp = (QuestionPoolFacade) poolsAvailable.get(i);
      selection.setLabel(qp.getDisplayName());
      selection.setValue("" + qp.getQuestionPoolId());
      list.add(selection);
    }
