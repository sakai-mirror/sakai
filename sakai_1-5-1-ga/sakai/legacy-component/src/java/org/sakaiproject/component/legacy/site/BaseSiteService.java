/**********************************************************************************
*
* $Header: /cvs/sakai/legacy-component/src/java/org/sakaiproject/component/legacy/site/BaseSiteService.java,v 1.54.2.1 2005/04/06 06:05:24 lance.indiana.edu Exp $
*
***********************************************************************************
*
...
public abstract class BaseSiteService implements SiteService, StorageUser
{
	...
	*/
	public boolean isSpecialSite(String site)
	{
		...
		// deal with a reference
		if (site.startsWith(siteReference("!")) && !site.equals(siteReference("!admin")))
			return true;
		...
		// deal with an id
		if (site.startsWith("!") && !site.equals("!admin"))
			return true;
	...
	*/
	 ...
	 * @param updateUsers The set of User objects who have update access.
	 * @param visitUnpUsers The set of User objects who have visit unpublished access.
	 * @param visitUsers The set of User objects who have visit access.
	 */
...
//			return ServerConfigurationService.getPortalUrl() + "?site=" + m_id;
		...
		*/
		...
		/**
		 * Access the site's properties, pages, each page's properties and tools, each tool's properties,
		 * to assure they are fully read in and not lazy.
		 */
		protected void readFully()
		{
			...
			for (Iterator pages = this.getPages().iterator(); pages.hasNext();)
			{
				...
				{
					BaseToolConfigurationEdit tool = (BaseToolConfigurationEdit) tools.next();
					tool.getProperties();
				}
			}
		}
	...
	public class BaseSiteEdit extends BaseSite implements SiteEdit, SessionStateBindingListener
	{
		...
		*/
		...
		public BaseSiteEdit(String id, String title, String type, String shortDesc, String description, String iconUrl,
							String infoUrl, String skin, boolean published, boolean joinable, boolean pubView,
							String joinRole, boolean isSpecial, boolean isUser,
		...
		* @param name The site's short description.
		*/
	...
	*******************************************************************************/
	...
	protected class BaseSitePageEdit implements SitePageEdit
	{
		...
		*/
		 ...
		 * @param layout The layout as a string ("0" or not currently supported).
		 */
		...
		*/
		public ToolConfigurationEdit getToolEdit(String id)
		{
			return (ToolConfigurationEdit) ((ResourceVector) getTools()).getById(id);
...
//			return ServerConfigurationService.getPortalUrl() + sitePageReference(m_site.getId(), m_id);
	...
	*******************************************************************************/
	...
	protected class BaseToolConfigurationEdit implements ToolConfigurationEdit
	{
		...
		*/
		 ...
		 * ReConstruct
		 * @param page The page in which this tool lives.
		 * @param id The tool (placement) id.
		 * @param toolId The id (registration code) of the tool to place here.
		 * @param title The tool title.
		 * @param layoutHints The layout hints.
		 */
		protected BaseToolConfigurationEdit(SitePage page, String id, String toolId, String title, String layoutHints)
		{
			m_page = (BaseSitePageEdit) page;
			m_id = id;
			m_toolId = toolId;
		 ...
		 * @param title The tool title.
		 * @param layoutHints The layout hints.
		 * @param pageId The page id in which this tool lives.
		 * @param siteId The site id in which this tool lives.
		 * @param skin The site's skin.
		 */
		protected BaseToolConfigurationEdit(String id, String toolId, String title, String layoutHints, String pageId, String siteId, String skin)
		{
			m_page = null;
			m_id = id;
			m_toolId = toolId;
		...
		*/
		public ResourcePropertiesEdit getPropertiesEdit()
		{
			...
			{
				((BaseResourcePropertiesEdit) m_properties).setLazy(false);
				m_storage.readToolProperties(this, m_properties);
			}
		...
		*/
		public String getUrl()
		{
			if (m_page == null)
			{
				return ServerConfigurationService.getAccessUrl() + siteToolReference(m_siteId, m_id);
			}
		...
		*/
		public String getReference()
		{
			if (m_page == null)
			{
				return siteToolReference(m_siteId, m_id);
			}
	...
	*******************************************************************************/
	 ...
	 * @param sort A SortType indicating the desired sort.  For no sort, set to SortType.NONE.
		...
		/**
		 * Access the ToolConfiguration that has this id, if one is defined, else return null.
		 * The tool may be on any SitePage in any site.
		 * @param id The id of the tool.
		 * @return The ToolConfiguration that has this id, if one is defined, else return null.
		 */
		public ToolConfiguration findTool(String id);
		...
		*/
		 ...
		 * @param site The site for which properties are desired.
		 */
	...
	*/
	...
	/**
	 * Adjust a skin to the default, if missing, and to have the proper extension, if needed.
	 * @param skin The skin value to adjust.
	...
	protected String adjustSkin(String skin, boolean published)
	{
		if (!published)
		{
			return ServerConfigurationService.getString("skin.default.unpublished", "pro.css");
		}

		if ((skin == null) || (skin.length() == 0))
		{
			skin = ServerConfigurationService.getString("skin.default", "sakai_core.css");
		}

		int pos = skin.indexOf(".css");

		// if totally missing, or not at the end, add it
		if ((pos == -1) || (pos != skin.length()-4))
		{
			skin += ".css";
		}
