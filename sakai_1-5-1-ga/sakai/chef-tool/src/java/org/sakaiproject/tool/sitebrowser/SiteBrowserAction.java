/**********************************************************************************
*
* $Header: /cvs/sakai/chef-tool/src/java/org/sakaiproject/tool/sitebrowser/SiteBrowserAction.java,v 1.24.2.2 2005/03/21 15:19:11 lance.indiana.edu Exp $
*
***********************************************************************************
*
	...
	extends PagedResourceActionII
{
	...
	private final static int NEW_DEFAULT_PAGE_SIZE = 5;
...
*/		
	...
	*/
	private String buildVisitContext(SessionState state, Context context)
		{
			int listSize = sizeResources(state);
			List sites = readResourcesPage(state, 1, listSize+1);
			...
			try
			{
				...
				if (goNext) // coming from by clicking "next msg"
				{
					if (pos+1 < sites.size()) pos++;
					site = (Site)sites.get(pos);
					if (site != null)
						state.setAttribute("siteId", site.getId());
				
				}
				else if (goPrev) // coming from by clicking "prev msg"
				{
					if (pos-1 >= 0) pos--;
					site = (Site)sites.get(pos);
					if (site != null)
				...
				boolean goPrevButton = state.getAttribute(STATE_PREV_EXISTS) != null;
				context.put("goPrevButton", Boolean.toString(goPrevButton));
				boolean goNextButton = state.getAttribute(STATE_NEXT_EXISTS) != null;
				context.put("goNextButton", Boolean.toString(goNextButton));
