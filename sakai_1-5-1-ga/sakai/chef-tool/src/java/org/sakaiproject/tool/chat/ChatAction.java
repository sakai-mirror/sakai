/**********************************************************************************
*
* $Header: /cvs/sakai/chef-tool/src/java/org/sakaiproject/tool/chat/ChatAction.java,v 1.23 2005/02/11 21:21:59 janderse.umich.edu Exp $
*
***********************************************************************************
*
...
import org.sakaiproject.util.delivery.FocusDelivery;
	...
	extends VelocityPortletPaneledAction
{
	...
	private static final String PERMISSION_DELETE_MESSAGE = "You do not have permission to delete this chat message.";
	...
	*/
	protected void initState(SessionState state, VelocityPortlet portlet, JetspeedRunData rundata)
	{
		...
		if (state.getAttribute(STATE_CHANNEL_REF) == null)
		{
			...
			if (channel == null)
			{
				channel = ChatService.channelReference(PortalService.getCurrentSiteId(), SiteService.MAIN_CONTAINER);
			}
			...
			if (state.getAttribute(STATE_OBSERVER) == null)
			{
				...
				state.setAttribute(STATE_OBSERVER, new ChatObservingCourier(deliveryId, elementId, pattern, wantsBeeps));
			}
			...
			if (state.getAttribute(STATE_CHAT_PRESENCE_OBSERVER) == null)
			{
				...
				// the html element to update on delivery
				String elementId = mainPanelUpdateId(portlet.getID()) + "." + PRESENCE_PANEL;

				// location is current portal
				String location = PortalService.getCurrentLocationId(portlet.getID());
				
				// setup an observer to notify us
				PresenceObservingCourier observer2 = new PresenceObservingCourier(deliveryId, elementId, location, true);

				// PresenceObservingCourier.FRAME_PRESENCE, instead of elementId???
				state.setAttribute(STATE_CHAT_PRESENCE_OBSERVER, observer2);

				if (Log.getLogger("chef").isDebugEnabled())
					Log.debug("chef", this + ": observer: " + observer2 + " elementId: " + elementId
	...
	*/
	private void updateObservationOfChannel(SessionState state, String peid)
	{
		...
		// update the observer looking for changes in chat presence
	...
	*/
	...
	{
		state.setAttribute(STATE_MORE_MESSAGES_LABEL, "Show All Messages");
		...
		if(filter_type.equals(FILTER_ALL))
		{
			...
			state.setAttribute(STATE_FEWER_MESSAGES_LABEL, "Show Past " + DEFAULT_DAYS + " Days");
		...
		else if(filter_type.equals(FILTER_TODAY))
		{
			...
			state.setAttribute(STATE_FEWER_MESSAGES_LABEL, "Show Today's Messages");
		...
		else if(filter_type.equals(FILTER_BY_NUMBER))
		{
			...
			state.setAttribute(STATE_FEWER_MESSAGES_FILTER, new SelectMessagesByNumber(number));
			state.setAttribute(STATE_FEWER_MESSAGES_LABEL, "Show Last " + number + " Messages");
			state.setAttribute(STATE_MESSAGE_FILTER, state.getAttribute(STATE_FEWER_MESSAGES_FILTER));
		}
		else if(filter_type.equals(FILTER_BY_TIME))
		{
			...
			state.setAttribute(STATE_FEWER_MESSAGES_LABEL, "Show Past " + number + " Days");
			state.setAttribute(STATE_MESSAGE_FILTER, state.getAttribute(STATE_FEWER_MESSAGES_FILTER));
		}
		else
		{
	...
	**/	
	public void doToggle_date_display(RunData runData, Context context)
	{
		...
		String address = ((ChatObservingCourier) state.getAttribute(STATE_OBSERVER)).getDeliveryId();
		String elementID = ((ChatObservingCourier) state.getAttribute(STATE_OBSERVER)).getElementId();
		CourierService.deliver(address, elementID);

		// schedule a delivery to return focus to Control panel
		String mainPanelId = mainPanelUpdateId(peid);
		String[] focusPath = {mainPanelId, CONTROL_PANEL, FORM_MESSAGE};
	...
	**/	
	...
	{
		toggleState(runData, STATE_DISPLAY_USER);
	...
	*/
										...
										SessionState state)
	{
		...
		bar.add( new MenuEntry((displayDate ? "Hide Date" : "Show Date"), null, true,
		...
		bar.add( new MenuEntry((displayTime ? "Hide Time" : "Show Time"), null, true,
		...
		if (ServerConfigurationService.getBoolean("java.beep", false))
		{
			...
			bar.add( new MenuEntry((soundAlert ? "Turn OFF New Message Alert" : "Turn ON New Message Alert"), null, true,
...
//			}
		...
		((ChatObservingCourier) state.getAttribute(STATE_OBSERVER)).justDelivered();
...
//		long startTime = System.currentTimeMillis();
		...
		Iterator aChannel = ChatService.getChannelIds(siteID).iterator();
...
//			}
	...
	public void doUpdate(RunData data, Context context)
	{
		...
		if (newChannel != null && newChannel.equals(NEW_CHAT_CHANNEL))
		{
			...
			catch(IdInvalidException e)
			{
				...
				addAlert(state, "You entered \"" + newChannel + "\" for Chat room name. Chat room name" + e.getMessage());
				return;
			}
		}
 		if (newChannel != null && !newChannel.equals(currentChannel))
		{
			state.setAttribute(STATE_CHANNEL_REF, ChatService.channelReference(siteID, newChannel));
			if (Log.getLogger("chef").isDebugEnabled())
			...
			tool.setTitle("Chat Room: \"" + newChannel + "\"");
		...
		if ( filter_type != null )
		{
			if( filter_type.equals(FILTER_ALL))
			{
				if(!filter_type.equals((String) state.getAttribute(STATE_FILTER_TYPE)))
				{
					...
					// update the tool config
					ToolConfigurationEdit tool = (ToolConfigurationEdit) state.getAttribute(STATE_TOOL_EDIT);
					tool.getPropertiesEdit().addProperty(PARAM_FILTER_TYPE, (String)state.getAttribute(STATE_FILTER_TYPE));
					tool.getPropertiesEdit().addProperty(PARAM_FILTER_PARAM, (String)state.getAttribute(STATE_FILTER_PARAM));
				}
			}
			else if( filter_type.equals(FILTER_BY_TIME))
			{
	...
	*/
	...
	public void doDeleteMessage(RunData data, Context context)
	{
		...
		try
		{
			...
			message.setBody("Chat message deleted by " + UserDirectoryService.getCurrentUser().getDisplayName() + " on " + (TimeService.newTime()).toStringLocalFull());
	...
	*/
	public void doPermissions(RunData data, Context context)
	{
		...
		state.setAttribute(PermissionsAction.STATE_DESCRIPTION, "Set permissions for Chat room in worksite "
