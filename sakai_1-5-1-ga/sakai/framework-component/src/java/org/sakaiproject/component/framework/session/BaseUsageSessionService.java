/**********************************************************************************
*
* $Header: /cvs/sakai/framework-component/src/java/org/sakaiproject/component/framework/session/BaseUsageSessionService.java,v 1.31 2005/02/23 21:10:13 ggolden.umich.edu Exp $
*
***********************************************************************************
*
...
public abstract class BaseUsageSessionService implements UsageSessionService
{
	...
	protected final String M_sessionKey = USAGE_SESSION_KEY;
	...
	*******************************************************************************/
	...
	public UsageSession startSession(String userId, String remoteAddress, String userAgent)
	{
		...
		if (hsession != null)
		{
			...
			try
			{
				...
				// mark it as bound so if we finalize it and it's still open we will unbind it properly and close the session
				((BaseUsageSession) session).m_bound = true;
	...
	public SessionState getSessionState(String key)
	{
		...
		if (hsession != null)
		{
			...
			{
				m_logger.warn(this + ".getSessionState(): IllegalStateException: key: " + key + " session: " + hsession.getId());
				return null;
			}
		}
				
		m_logger.warn(this + ".getSessionState(): no session: key: " + key);
		return null;
	}
	...
	*******************************************************************************/
	...
	protected class BaseUsageSession implements UsageSession, HttpSessionBindingListener
	{
		...
		/** This object is bound to the http session, and on finalize if not closed need to be closed. */
		...
		public void valueUnbound(HttpSessionBindingEvent hsbe)
		{
			try
			{
				...
				if (!isClosed())
				{
					...
					// mark unbound
					if (!m_bound) m_logger.warn("BaseUsageSessionService.valueUnbound: object was not marked bound: "
												+ this.toString() + " @ " + Integer.toHexString(hashCode()));
					m_bound = false;
				}
			}
			finally
		...
		*/
			...
			throws Throwable
		{
			try
			{
				// just in case valueUnbound was not called
				if (m_bound && (m_start != null) && (m_end != null) && !isClosed())
				{
					m_logger.warn("BaseUsageSessionService.BaseUsageSession.finalize(): open bound session: " + m_user + " - "
									+ m_id + " - " + m_server + " @ " + Integer.toHexString(hashCode()));
					valueUnbound(null);
				}
			}
			catch (Throwable t)
			{
				m_logger.warn("BaseUsageSessionService.BaseUsageSession.finalize(): " + t);
			}
			finally
			{
				super.finalize();
	...
	*******************************************************************************/
	...
	protected class BaseSessionState implements SessionState, HttpSessionBindingListener
	{
		...
		{
			return setAttribute((String) key, value);
		}
		...
		*/
			...
			throws Throwable
		{
			try
			{
				...
				{
					m_logger.warn("BaseUsageSessionService.BaseSessionState.finalize: still-bound sesion state: " + m_key);
					valueUnbound(null);
				}
			}
			catch (Throwable t)
			{
				m_logger.warn("BaseUsageSessionService.BaseSessionState.finalize: " + t);
			}
			finally
			{
				super.finalize();
