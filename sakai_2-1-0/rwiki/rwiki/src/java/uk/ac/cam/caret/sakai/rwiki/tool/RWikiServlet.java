...
public class RWikiServlet extends HttpServlet {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response)
        ...
        /*
         1. Every time we pass this point, we must store some state. This should be
            the URL of the page.
         2. If we return, and the page being requested is the default page, we should
         check to see if there is state, and use that state instead.
         3. Storing the request object could be expensive and unnecessary.
         4. Storing anytihng else will require co-ordination underneath.
         5. When we hit a post, we cant re-post, but we must generate the same output page.
         
         Solution:
         We cant store request as it has loads of Framework stuf done to it in the portal
         Store the URL, and redirect on restore
         Pages that do POSTS need to set the target URL
     ...
     * the request attribute RWikiServlet.REQUEST_STATE_RESTORED to Boolean.TRUE and a
     * Thread local named RWikiServlet.REQUEST_STATE_RESTORED to Boolean.TRUE. These
     * MUST be checked by anything that modifies state, to ensure that a reinitialisation
     * of Tool state does not result in a repost of data.
     * @param request
     * @return
     */
    ...
    /**
     * Check to see if the reques represents the Tool default page. This is not the 
     * same as the view Home. It is the same as first entry into a Tool or when the page is refreshed
     * @param request
    ...
    /**
     * Check to see if the request represents a page that can act as a restor point.
     * @param request
