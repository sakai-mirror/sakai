 ...
 *
 * $Header: /opt/CVS/osp2.x/homesComponent/src/java/org/theospi/metaobj/repository/mgt/FileExtensionFilter.java,v 1.1 2005/06/29 18:36:41 chmaurer Exp $
 * $Revision: 1.1 $
...
public class FileExtensionFilter extends MimeTypeFilter {
   protected final transient Log logger = LogFactory.getLog(getClass());

   private List acceptedExtensions;

   public boolean allowSelectNode(Node nodeToSelect, Map params) {
      if (super.allowSelectNode(nodeToSelect, params)) {
         for (Iterator i=getAcceptedExtensions().iterator();i.hasNext();) {
            if (nodeToSelect.getPath().endsWith("." + i.next().toString().toLowerCase())) {
               return true;
            }
         }
