/**********************************************************************************
*
* $Header: /cvs/sakai/legacy-component/src/java/org/sakaiproject/component/legacy/user/BaseUserDirectoryService.java,v 1.38 2004/10/14 18:40:07 ggolden.umich.edu Exp $
*
***********************************************************************************
*
...
/**
* <p>BaseUserDirectoryService is a CHEF user directory services implemented as a Turbine Service.</p>
* 
...
public abstract class BaseUserDirectoryService implements UserDirectoryService, StorageUser, CacheRefresher
{
	...
	*/
	...
	public User authenticate(String id, String password)
	{
		...
		// 1. container authentication - only if enabled, and if password is null
		//    authenticate if the session user matches the request (container) user
		if ((ServerConfigurationService.getBoolean("container.auth",false)) && (password == null))
		{
			// get the current request
			HttpServletRequest req = (HttpServletRequest) CurrentService.getInThread(HttpServletRequest.class.getName());

			if (id.equals(req.getRemoteUser()))
			{
				authenticated = true;
				// m_logger.info(" *** UserDirectory.authenticate: id: " + id + " allowing container authorized: " + authenticated);
			}
		}
		...
		// 3. check our provider, if any, if not yet authenticated
	...
	public class BaseUserEdit implements UserEdit, SessionStateBindingListener
	{
		...
		*/
							...
							String createdBy, Time createdOn, String modifiedBy, Time modifiedOn)
		{
			...
			ResourcePropertiesEdit props = new BaseResourcePropertiesEdit();
			m_properties = props;
			props.addProperty(ResourceProperties.PROP_CREATOR, createdBy);
			props.addProperty(ResourceProperties.PROP_CREATION_DATE, createdOn.toString());
			props.addProperty(ResourceProperties.PROP_MODIFIED_BY, modifiedBy);
			props.addProperty(ResourceProperties.PROP_MODIFIED_DATE, modifiedOn.toString());
		}
