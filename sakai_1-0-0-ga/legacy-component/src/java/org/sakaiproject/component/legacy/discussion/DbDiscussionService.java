/**********************************************************************************
*
* $Header: /cvs/sakai/legacy-component/src/java/org/sakaiproject/component/legacy/discussion/DbDiscussionService.java,v 1.14 2004/10/07 08:08:35 janderse.umich.edu Exp $
*
***********************************************************************************
*
...
/**
* <p>DbDiscussionService fills out the BaseDiscussionService with a database implementation.</p>
* <p>The sql scripts in src/sql/chef_discussion.sql must be run on the database.</p>
* 
		...
		implements Storage
	{
		...
		*/
		public DbStorage(StorageUser user)
		{
			...
			"MESSAGE_DATE", "OWNER", "DRAFT", FIELDS, m_locksInDb, "channel", "message", user, m_sqlService);
				...
				new SqlReader()
				{
					...
					public Object readSqlResultRecord(ResultSet result)
					{
						try
						{
							...
							if (!ok)
								m_logger.info(this + ".convertToDraftPlus: channel: " + channelId + " message: "
										+ messageId + " owner: " + owner + " draft: " + draft
										+ " category: " + category + " replyTo: " + replyTo + " ok: " + ok);

							count++;
							if (count % 100 == 0)
							{
								m_logger.debug(this + ".convertToDraftPlus: " + count);
							}
							return null;
						}
