/**********************************************************************************
*
* $Header: /cvs/sakai/legacy-component/src/java/org/sakaiproject/component/legacy/assignment/BaseAssignmentService.java,v 1.39 2004/10/07 01:57:37 ggolden.umich.edu Exp $
*
***********************************************************************************
*
...
//			}
	...
	*/
	public void commitEdit(AssignmentEdit assignment)
	{
		...
		assignment.setTimeLastModified(TimeService.newTime());
...
//			}
	...
	*/
	public void commitEdit(AssignmentContentEdit content)
	{
		...
		content.setTimeLastModified(TimeService.newTime());
...
//			}
	...
	*/
		...
		throws IdUnusedException, PermissionException
	{
		...
		{
			Assignment a = (Assignment) assignmentsVector.get(i);
		...
		List members = allowAddSubmissionUsers(((Assignment) assignmentsVector.get(0)).getReference());
		for (Iterator it= members.iterator(); it.hasNext(); )
		{
			...
			for (int i = 0; i < assignmentsVector.size(); i++)
			{
				...
				try
				{
					AssignmentSubmission s = getSubmission(a.getId(), member);
					...
					if ((s!=null) && (s.getGraded()) && (s.getGradeReleased()))
					{
						if (a.getContent().getTypeOfGrade() == 3)
						{
							...
							cell = row.createCell(cellNum++);
							cell.setCellType(0); 
							String grade = s.getGradeDisplay();
				...
				{
					throw new PermissionException(UsageSessionService.getSessionUserId(), SECURE_ACCESS_ASSIGNMENT_SUBMISSION, a.getId());
				}
			}	//	while
		}	//	for
							...
							try
							{
								...
								for (int j=0; j < attachments.size(); j++)
								{
									...
									catch (PermissionException e)
									{
										...
										throw new PermissionException(UsageSessionService.getSessionUserId(), ContentHostingService.EVENT_RESOURCE_READ, r.getId());
									}
									catch (IdUnusedException e)
									{
										if (m_logger.isDebugEnabled())
								...
								for (int i3 = 0; i3 < length3; i3++)
								{
									...
									if (child3.getNodeType() == Node.ELEMENT_NODE)
									{
										...
										else if (element3.getTagName().equals("content"))
										{
											...
											for(int i5 = 0; i5 < length5; i5++)
											{
												...
												if (child5.getNodeType() == Node.ELEMENT_NODE)
												{
													...
													if (element5.getTagName().equals("attachment"))
													{
														...
														el3clone.setAttribute(attachmentString, oldUrl);
														attCount++;
											...
											AssignmentContentEdit contentEdit = mergeAssignmentContent(el3clone);
	...
	*******************************************************************************/
		...
		implements Assignment
	{
		...
		public Element toXml(Document doc, Stack stack)
		{
			...
			assignment.setAttribute("datecreated", getTimeString(m_timeCreated));
			assignment.setAttribute("lastmod", getTimeString(m_timeLastModified));
	...
	*******************************************************************************/
		...
		implements AssignmentContent
	{
		...
		public BaseAssignmentContent(Element el)
		{
			...
			m_instructions = StringUtil.trimToNull(Xml.decodeAttribute(el, "instructions-formatted"));
			...
			for(int i = 0; i < length; i++)
			{
				...
				else if(element.getTagName().equals("instructions-formatted") || element.getTagName().equals("instructions"))
				{
		...
		public Element toXml(Document doc, Stack stack)
		{
			...
			Xml.encodeAttribute(content, "instructions-formatted", m_instructions);

			// TODO: [DONE] store the non-formatted for CHEF compatibility
			Xml.encodeAttribute(content, "instructions", FormattedText.convertFormattedTextToPlaintext(m_instructions));
	...
	*******************************************************************************/
	...
	public class BaseAssignmentSubmission implements AssignmentSubmission
	{
		...
		public BaseAssignmentSubmission(Element el)
		{
			...
			// %%% JANDERSE attribute names changed to reflect switch to formatted text from plaintext 
			m_submittedText = StringUtil.trimToNull(Xml.decodeAttribute(el, "submittedtext-formatted"));
			m_feedbackComment = StringUtil.trimToNull(Xml.decodeAttribute(el, "feedbackcomment-formatted"));
			m_feedbackText = StringUtil.trimToNull(Xml.decodeAttribute(el, "feedbacktext-formatted"));
			
			// %%% JANDERSE for backwards compatibility - convert old plaintext to new formatted text
			if (m_submittedText == null) m_submittedText = FormattedText.convertPlaintextToFormattedText(Xml.decodeAttribute(el, "submittedtext"));
			if (m_feedbackComment == null) m_feedbackComment = FormattedText.convertPlaintextToFormattedText(Xml.decodeAttribute(el, "feedbackcomment"));
			if (m_feedbackText == null) m_feedbackText = FormattedText.convertPlaintextToFormattedText(Xml.decodeAttribute(el, "feedbacktext"));
		...
		public Element toXml(Document doc, Stack stack)
		{
			...
			Xml.encodeAttribute(submission, "submittedtext-formatted", m_submittedText);

			// TODO: [DONE] store the non-formatted for CHEF compatibility
			Xml.encodeAttribute(submission, "submittedtext", FormattedText.convertFormattedTextToPlaintext(m_submittedText));

				// SAVE THE FEEDBACK COMMENT
			Xml.encodeAttribute(submission, "feedbackcomment-formatted", m_feedbackComment);

			// TODO: [DONE] store the non-formatted for CHEF compatibility
			Xml.encodeAttribute(submission, "feedbackcomment", FormattedText.convertFormattedTextToPlaintext(m_feedbackComment));

				// SAVE THE FEEDBACK TEXT
			Xml.encodeAttribute(submission, "feedbacktext-formatted", m_feedbackText);

			// TODO: [DONE] store the non-formatted for CHEF compatibility
			Xml.encodeAttribute(submission, "feedbacktext", FormattedText.convertFormattedTextToPlaintext(m_feedbackText));
