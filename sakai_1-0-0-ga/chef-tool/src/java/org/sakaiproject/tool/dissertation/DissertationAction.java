/**********************************************************************************
*
* $Header: /cvs/sakai/chef-tool/src/java/org/sakaiproject/tool/dissertation/DissertationAction.java,v 1.23 2004/10/08 00:00:23 rwellis.umich.edu Exp $
*
***********************************************************************************
*
...
//	** One time initialization of CandidatePath type attribute */
	...
	*/
										...
										SessionState state)
	{
		...
		//IF SCHOOL SITE, SET CURRENT DISSERTATION STEPS TYPE
		if(currentSite.equals((String)state.getAttribute(STATE_SCHOOL_SITE)))
		{
			if(((String)state.getAttribute(STATE_DISSERTATION_TYPE)).equals(DissertationService.DISSERTATION_TYPE_MUSIC_PERFORMANCE))
				stepsType = DissertationService.DISSERTATION_TYPE_MUSIC_PERFORMANCE;
			else
				stepsType = DissertationService.DISSERTATION_TYPE_DISSERTATION_STEPS;
		}
		
		Vector letters = (Vector)getLetters(currentSite, stepsType);
	...
	**/
	public void doAddnew_dissertation_step (RunData data, Context context)
	{
		...
		try
		{
			...
			{
				//CURRENT DISSERTATION IS SCHOOL'S - ADDING TO ALL DISSERTATIONS
				...
				for(int x = 0; x < allDissertations.size(); x++)
				{
					...
					// apply to all dissertations of the same type as current dissertation
					if (dissertation.getType().equals(tempDissertation.getType()))
					{
				...
				{
					//RETRO IS ON !!!!! - ADDING TO ALL CANDIDATE PATHS
					...
					for(int y = 0; y < allPaths.size(); y++)
					{
						...
						if (dissertation.getType().equals(tempPath.getType()))
						{
							try
							{
			...
			try
			{
				...
				if(currentDissertation.getSite().equals(DissertationService.getSchoolSite()))
				{
					...
					for(int y = 0; y < allChecklistDefs.size(); y++)
					{
						...
						if(currentDissertation.getType().equals(tempDiss.getType()))
						{
				...
				if(retro.booleanValue())
				{
					for(int v = 0; v < candidatePathsToUpdate.size(); v++)
					{
						...
						if(currentDissertation.getType().equals(tempPath.getType()))
						{
...
// ******************************************************************************************************************************************************************
	...
	**/
	public void doAlphabetical_choice(RunData data, Context context)
	{
		...
		User[] allUsers = DissertationService.getAllUsersForSite(department, stepsType);
		...
		{
			User[] usersByLetter = getUsersByLetter(allUsers, letterChosen);
			Vector sortedUsers = new Vector();
			for(int x = 0; x < usersByLetter.length; x++)
			{
				sortedUsers.add(usersByLetter[x]);
			}
			Collections.sort(sortedUsers, new UserComparator());
			User[] sortedUsersByLetter = new User[sortedUsers.size()];
			for(int y = 0; y < sortedUsers.size(); y++)
			{
				sortedUsersByLetter[y] = (User)sortedUsers.get(y);
			}
			state.setAttribute(STATE_USERS_LIST, sortedUsersByLetter);
		}
		state.setAttribute(STATE_MODE, MODE_ADMIN_ALPHABETICAL_CANDIDATE_CHOOSER);
	}
	...
	*/
	protected void initState(SessionState state, VelocityPortlet portlet, JetspeedRunData rundata)
	{
		...
		if (state.getAttribute(STATE_DISSERTATION_TYPE)==null)
		{
			state.setAttribute(STATE_DISSERTATION_TYPE, DissertationService.DISSERTATION_TYPE_DISSERTATION_STEPS);
		}
		...
		// CANDIDATE, COMMITTEE, ADMINISTRATOR OR DEAN ROLE DECISION TREE
		if(state.getAttribute(STATE_USER_ROLE) == null)
		{	
			if((emplid.equals("")) || (!DissertationService.getIsCandidate(currentUser.getId())))
			{
				...
				if(!allowAddDiss)
				{
					...
					if((currentSite.equals(schoolSite)) || (currentSite.matches("^diss[0-9]*$")))
					{
						...
						if((state.getAttribute(STATE_LETTERS) == null) || (state.getAttribute(STATE_CURRENT_DISSERTATION_STEPS) == null)) 
						{
					...
					try
					{
						...
						if(dissertation == null)
						{
							...
							try
							{
								...
								if(DissertationService.getIsMusicPerformanceCandidate(currentUser.getId()))
								{
							...
							if(dissertation == null)
							{
								...
								modeString = MODE_CANDIDATE_NO_DISSERTATION;
								return;
							}
							try
							{
		...
		for(int x = 0; x < allCandidatePaths.size(); x++)
		{
			...
			if(tempPath.getType()==null || ((String)tempPath.getType()).equals(""))
			{
				...
				try
				{
					...
					parentSite = info.getParentSiteId();
					try
					{
						pathEdit = DissertationService.editCandidatePath(tempPath.getReference());
	...
	*/
	...
	/**
	* Populate the Candidate Chooser letters and Rackham/Dept Site DissertationSteps
	* @param state The state object.
	...
	*/
	protected void initDissertation(SessionState state, String currentSite)
	{
		...
		/*
		// SET UP THE CANDIDATE-CHOOSER LETTERS
		if (state.getAttribute(STATE_LETTERS) == null)
		{
			state.setAttribute(STATE_LETTERS, getLetters(currentSite));
		}
		*/
			...
			else
			{
				...
				try
				{
					...
					if(musicPerformanceSite)
					{
						...
						//schoolDissertation = DissertationService.getDissertationForSite(currentSite, DissertationService.DISSERTATION_TYPE_MUSIC_PERFORMANCE);
					...
					else
					{
						...
						//schoolDissertation = DissertationService.getDissertationForSite(currentSite, DissertationService.DISSERTATION_TYPE_DISSERTATION_STEPS);
	...
	*/
	private TemplateStep[] getTemplateSteps(Dissertation dissertation, SessionState state)
	{
		...
		if(dissertation.getSite().equals(DissertationService.getSchoolSite()))
		{
			...
			activeCandidates = DissertationService.getActivePaths();
			//ACTIVE CANDIDATES
		}
		else
		{
	...
	*/
	private Vector getLetters(String site, String stepsType)
	{
		Vector retVal = new Vector();
		...
		lettersUsers = getUsersByLetter(allusers, "B");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);

		aCarrier = new LetterCarrier("C");
		lettersUsers = getUsersByLetter(allusers, "C");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);

		aCarrier = new LetterCarrier("D");
		lettersUsers = getUsersByLetter(allusers, "D");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);

		aCarrier = new LetterCarrier("E");
		lettersUsers = getUsersByLetter(allusers, "E");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);

		aCarrier = new LetterCarrier("F");
		lettersUsers = getUsersByLetter(allusers, "F");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);

		aCarrier = new LetterCarrier("G");
		lettersUsers = getUsersByLetter(allusers, "G");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);

		aCarrier = new LetterCarrier("H");
		lettersUsers = getUsersByLetter(allusers, "H");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);

		aCarrier = new LetterCarrier("I");
		lettersUsers = getUsersByLetter(allusers, "I");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);

		aCarrier = new LetterCarrier("J");
		lettersUsers = getUsersByLetter(allusers, "J");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("K");
		lettersUsers = getUsersByLetter(allusers, "K");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("L");
		lettersUsers = getUsersByLetter(allusers, "L");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("M");
		lettersUsers = getUsersByLetter(allusers, "M");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("N");
		lettersUsers = getUsersByLetter(allusers, "N");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("O");
		lettersUsers = getUsersByLetter(allusers, "O");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("P");
		lettersUsers = getUsersByLetter(allusers, "P");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("Q");
		lettersUsers = getUsersByLetter(allusers, "Q");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("R");
		lettersUsers = getUsersByLetter(allusers, "R");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("S");
		lettersUsers = getUsersByLetter(allusers, "S");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("T");
		lettersUsers = getUsersByLetter(allusers, "T");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("U");
		lettersUsers = getUsersByLetter(allusers, "U");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("V");
		lettersUsers = getUsersByLetter(allusers, "V");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("W");
		lettersUsers = getUsersByLetter(allusers, "W");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("X");
		lettersUsers = getUsersByLetter(allusers, "X");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("Y");
		lettersUsers = getUsersByLetter(allusers, "Y");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
		
		aCarrier = new LetterCarrier("Z");
		lettersUsers = getUsersByLetter(allusers, "Z");
		if(lettersUsers.length == 0)
			aCarrier.setHasMembers(false);
		retVal.add(aCarrier);
