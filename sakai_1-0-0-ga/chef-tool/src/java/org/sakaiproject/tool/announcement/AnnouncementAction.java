/**********************************************************************************
*
* $Header: /cvs/sakai/chef-tool/src/java/org/sakaiproject/tool/announcement/AnnouncementAction.java,v 1.27 2004/10/07 01:57:36 ggolden.umich.edu Exp $
*
***********************************************************************************
*
...
public class AnnouncementAction extends VelocityPortletStateAction
{
	...
	private static final String NOTI_REQUIRED = "High - All participants";
	private static final String NOTI_OPTIONAL = "Low - Only participants who have opted in";
	private static final String NOTI_NONE = "None - No notification";
	...
	private static final String PERMISSIONS_BUTTON_CAPTION = "Permissions...";
	private static final String MERGE_BUTTON_HANDLER = "doMerge";
	private static final String MERGE_BUTTON_CAPTION = "Merge...";
...
//	} 	
	...
	protected void enableObserver(SessionState sstate, boolean enable)
	{
		...
		(ObservingCourier) sstate.getAttribute(
		VelocityPortletPaneledAction.STATE_OBSERVER);
		if (observer != null)
		{
			if (enable)
			{
	 ...
	 * @param state The AnnouncementActionState object.
	 * @param sstate The session state.
	 */
		...
		SessionState sstate)
	{
		...
		if (channelId == null)
		{
			...
			if (channelId == null)
			{
				...
				PortalService.getCurrentSiteId(), SiteService.MAIN_CONTAINER);
			}
		...
		try
		{
				...
				&& isOkayToDisplayMessageMenu(state))
			{
				...
				{
					// provide "announcements_messages" as an iterator on the messages
					context.put("ann_messages", channel.getMessages(null, true));

					menu_new = channel.allowAddMessage();

					List messages = getMessages(channel, null, true, state, portlet);
		...
		((EventsObservingCourier) sstate.getAttribute(STATE_OBSERVER)).justDelivered();
											...
											SessionState state)
	{
		...
		Reference channelRef = new Reference(channelId);
				context.put("description", "Setting options for Announcements in worksite "
	 ...
	 * @param context The context object
	 * @param runData The RunData object
	 * @param state The ResourceActionState object
	 */
		...
		AnnouncementActionState state)
	{
		...
		if (pubview)
			context.put("IsPubView", "Yes");
		else
			context.put("IsPubView", "No");
		...
		SessionState sstate)
	{
		...
		// if this is an existing one
			...
			if (state.getStatus().equals("goToReviseAnnouncement"))
			{
				...
				context.put("pubviewset", new Boolean(RealmService.getPubViewInheritance(edit.getReference())));
				context.put("pubview", new Boolean(RealmService.getPubView(edit.getReference())));
		...
		SessionState sstate)
	{
		...
		try
		{
			...
			// find out about pubview
			context.put("pubviewset", new Boolean(RealmService.getPubViewInheritance(message.getReference())));
			context.put("pubview", new Boolean(RealmService.getPubView(message.getReference())));
		...
		catch (PermissionException e)
		{
			...
			addAlert(
				sstate,
				"You don't have permissions to access the message(s) - "
					+ e.toString());
		}
	...
	public void doShowmetadata(RunData rundata, Context context)
	{
		...
		// disable auto-updates while in view mode
		((EventsObservingCourier) sstate.getAttribute(STATE_OBSERVER)).disable();
	...
	public void doPost(RunData rundata, Context context)
	{
			...
			if (from.equals("revise"))
			{
				...
				if (subject.length() == 0)
				{
					...
					if (body.length() == 0)
					{
						addAlert(
							sstate,
							"You need to fill in the body of the announcement!");
					}
			}
		...
		if (sstate.getAttribute(STATE_MESSAGE) != null)
		{
			...
			// disable auto-updates while in view mode
			((EventsObservingCourier) sstate.getAttribute(STATE_OBSERVER))
				.disable();
		}
		else
		{
			...
			try
			{
				...
				if (state.getIsNewAnnouncement())
				{
					...
					boolean pubviewset = RealmService.getPubViewInheritance(msg.getReference());
					...
					if (!pubviewset)
					{
						try
						{
							RealmService.setPubView(msg.getReference(), pubview);
						}
			...
			catch (PermissionException e)
			{
				...
				addAlert(
					sstate,
					"You don't have permissions to create this announcement -"
						+ subject);
			}
		...
		while (delete_messages.hasNext())
		{
			try
			{
				...
				if (channel.allowRemoveMessage(message))
				{
					...
					// make sure auto-updates are enabled
					((EventsObservingCourier) sstate
						.getAttribute(STATE_OBSERVER))
						.enable();
				}
				else
				{
		...
		if (state.getIsListVM())
		{
			...
			if (messageReferences != null)
			{
				if (messageReferences.length > 1)
				{
					...
					// make sure auto-updates are enabled
					((EventsObservingCourier) sstate.getAttribute(STATE_OBSERVER)).enable();
					...
					if (messageReferences.length == 1)
					{
						...
						catch (PermissionException e)
						{
							if (Log.getLogger("chef").isDebugEnabled())
							Log.debug("chef", this + "announcementReviseFromMenu" + e);
						addAlert(sstate, "You don't have permissions to access the message(s) - "+ e.toString());
						}
						...
						addAlert(sstate, "This item is being edited by another user. Please try again later.");
							state.setIsListVM(false);
							state.setStatus("showMetadata");
	...
	public void doRevisepreview(RunData rundata, Context context)
	{
		...
		//{
			...
			if (subject.length() == 0)
			{
				...
				if (body.length() == 0)
				{
				addAlert(sstate, "You need to fill in the body of the announcement!");
				}
		//} // if (from.equals("revise"))
	...
	public void doAttachments(RunData data, Context context)
	{
		...
		String stateFromText = "the announcement";
		if (subject != null && subject.length() > 0)
		{
			stateFromText = "announcement " + '"' + subject + '"';
		}
	...
	public void doSavedraft(RunData rundata, Context context)
	{
		...
		else
		{
			...
			{
				addAlert(sstate, "you don't have permissions to create this announcement -"+ subject);
				if (Log.getLogger("chef").isDebugEnabled())
					Log.debug("chef", this +"doSavedraft()" + e);
			} // try-catch
	...
	*/
		...
		AnnouncementActionState.DisplayOptions displayOptions)
	{
		...
		if (!displayOptions.isShowOnlyOptionsButton())
		{
			...
			if (statusName != null)
			{
				...
				{
					bar.add(new MenuEntry("New...",null, menu_new,	MenuItem.CHECKED_NA,"doNewannouncement"));
					bar.add(new MenuEntry("Delete",null, menu_delete,MenuItem.CHECKED_NA,"doDeleteannouncement"));
					bar.add(new MenuEntry("Revise",null,menu_revise,MenuItem.CHECKED_NA,"doReviseannouncementfrommenu"));
				...
				{
					bar.add(new MenuEntry("New...",null,menu_new,MenuItem.CHECKED_NA,"doNewannouncement","announcementListForm"));
					bar.add(new MenuEntry("Delete",null,menu_delete,MenuItem.CHECKED_NA,"doDeleteannouncement","announcementListForm"));
					bar.add(new MenuEntry("Revise",null,menu_revise,MenuItem.CHECKED_NA,"doReviseannouncementfrommenu","announcementListForm"));
					//bar.add( new MenuEntry("Go home", null, true/* visible at all views */, MenuItem.CHECKED_NA, "doHome") );
					buttonRequiringCheckboxesPresent=true ;
	...
	protected void initState(SessionState state, VelocityPortlet portlet, JetspeedRunData rundata)
	{
		...
		if (state.getAttribute(STATE_OBSERVER) == null)
		{
			...
			updateObservationOfChannel(mergedAnnouncementList, rundata, state, annState);
		}
	 ...
	 * in watching.
	 * @param mergedAnnouncementList
	 * @param annState
	 * @param observer
	 */
	private void addMergedAnnouncementsToObserver(MergedList mergedAnnouncementList, AnnouncementActionState annState, EventsObservingCourier observer)
	{
	...
	*/
	public void doPermissions(RunData data, Context context)
	{
		...
		state.setAttribute(
			...
			"Set permissions for Announcements in worksite "
				+ SiteService.getSiteDisplay(channelRef.getContext()));
...
//		alert.doOptions(runData, context);
	...
	private void doMergeUpdate(RunData runData, Context context)
	{
		...
		if (mergedChannelList != null)
		{
			...
			mergedChannelList.loadFromRunData(runData);
		}
		else
		{
			if (Log.getLogger("chef").isDebugEnabled())
		...
		ToolConfigurationEdit tool =
			(ToolConfigurationEdit) sstate.getAttribute(STATE_TOOL_EDIT);

		// Make sure that the older, incorrectly named paramter is gone.
		tool.getPropertiesEdit().removeProperty(
				PORTLET_CONFIG_PARM_NON_MERGED_CHANNELS);
		

		if (mergedChannelList != null)
		{
			tool.getPropertiesEdit().addProperty(
		...
		SessionState sstate)
	{
		...
		displayOptions.saveProperties(tool.getPropertiesEdit());
