/**********************************************************************************
*
* $Header: /cvs/sakai/chef-tool/src/java/org/sakaiproject/tool/content/ResourcesAction.java,v 1.21 2004/09/30 20:20:57 ggolden.umich.edu Exp $
*
***********************************************************************************
*
...
extends VelocityPortletPaneledAction
{
	...
	private static final String BOOLEAN_TRUE_STRING = Boolean.TRUE.toString();
	private static final String BOOLEAN_FALSE_STRING = Boolean.FALSE.toString();
	...
	*/
										...
										SessionState state)
	{
		...
		try
		{
			...
			context.put ("collectionFlag", BOOLEAN_TRUE_STRING);
			...
			if (cutFlag.equals (BOOLEAN_TRUE_STRING))
			{
				context.put ("cutItems", state.getAttribute (STATE_CUT_IDS));
			}
			if (copyFlag.equals (BOOLEAN_TRUE_STRING))
			{
		...
		catch (IdUnusedException e)
		{
			...
			context.put ("collectionFlag", BOOLEAN_FALSE_STRING);
			buildMenu (portlet, context, data, state, allowAdd, allowRemove, allowReplace, allowRevise, emptyFolder, atHome);
		}
		catch(TypeException e)
		{
	...
	*/
											...
											SessionState state)
	{
		...
		if (((String) state.getAttribute(STATE_RESOURCES_MODE)).equalsIgnoreCase(RESOURCES_MODE_RESOURCES))
		{
			...
			boolean pubviewset = RealmService.getPubViewInheritance(collectionReference) || RealmService.getPubView(collectionReference);
	...
	*/
											...
											SessionState state)
	{
		...
		context.put ("copyright", state.getAttribute (STATE_ADD_DOCUMENT_COPYRIGHT));
	...
	*/
									...
									SessionState state)
	{
		...
		try
		{
			...
			if ((isCollection != null) && isCollection.equals (BOOLEAN_FALSE_STRING))
			{
		...
		if (((String) state.getAttribute(STATE_RESOURCES_MODE)).equalsIgnoreCase(RESOURCES_MODE_RESOURCES))
		{
			...
			boolean pubview = RealmService.getPubViewInheritance(ContentHostingService.getReference(id));
			if (!pubview) pubview = RealmService.getPubView(ContentHostingService.getReference(id));
	...
	*/
										...
										SessionState state)
	{
		...
		// for the resources of type URL or plain text, show the content also
		if (properties.getProperty (ResourceProperties.PROP_IS_COLLECTION).equals (BOOLEAN_FALSE_STRING))
		{
			String type = properties.getProperty (ResourceProperties.PROP_CONTENT_TYPE);
			if ((type.equalsIgnoreCase (MIME_TYPE_DOCUMENT_PLAINTEXT)) || (type.equalsIgnoreCase (MIME_TYPE_DOCUMENT_HTML)) || (type.equalsIgnoreCase (ResourceProperties.TYPE_URL)))
			{
		...
		if (state.getAttribute(STATE_PROPERTIES_COPYRIGHT_CHOICE) == null)
		{
			if (copyright.length() == 0)
			{
				//no copyright information
				state.setAttribute(STATE_PROPERTIES_COPYRIGHT_CHOICE, "");
			}
			else
			{			 
				if (copyright.equals((String) state.getAttribute (STATE_MY_COPYRIGHT)))
				{
					state.setAttribute(STATE_PROPERTIES_COPYRIGHT_CHOICE, "my copyright");
				}
				else
				{
					state.setAttribute(STATE_PROPERTIES_COPYRIGHT_CHOICE, "newcopyright");
				}
		
			}
		}

		if (state.getAttribute(STATE_PROPERTIES_COPYRIGHT) == null)
		{
		...
		if (((String) state.getAttribute(STATE_RESOURCES_MODE)).equalsIgnoreCase(RESOURCES_MODE_RESOURCES))
		{
			...
			// find out about pubview
			context.put("pubviewset", new Boolean(RealmService.getPubViewInheritance(ContentHostingService.getReference(id))));
			context.put("pubview", new Boolean(RealmService.getPubView(ContentHostingService.getReference(id))));
	...
	*/
	public void doNavigate ( RunData data )
	{
		...
		if (state.getAttribute (STATE_SELECT_ALL_FLAG)!=null && state.getAttribute (STATE_SELECT_ALL_FLAG).equals (BOOLEAN_TRUE_STRING))
		{
			state.setAttribute (STATE_SELECT_ALL_FLAG, BOOLEAN_FALSE_STRING);
		}
		
		if (state.getAttribute (STATE_EXPAND_ALL_FLAG)!=null && state.getAttribute (STATE_EXPAND_ALL_FLAG).equals (BOOLEAN_TRUE_STRING))
		{
			state.setAttribute (STATE_EXPAND_ALL_FLAG, BOOLEAN_FALSE_STRING);
		}
			...
			if (params.getString ("itemType").equals ("file"))
			{
				...
				if (s != null)
				{
					...
					for (int i=1; i <= (newFileNumber<oldFileNumber?newFileNumber:oldFileNumber); i++)
					{
						...
						String newcopyrighttext = params.getCleanString ("newcopyrighttext" + i);
			
						if (newcopyrighttext!=null)
						{
	...
	public void doAdd_file_basic ( RunData data)
	{
		...
		else
		{
			...
			if (repeatedNames_index >0)
			{
				...
				if (n.length ()>0)
				{
					addAlert(state,"Can not add files with the same title " + n + ". ");
					...
					if (state.getAttribute(STATE_MESSAGE) == null)
					{
						...
						for (int i=1; i <= fileNumber; i++)
						{
							if (((String) titles.get(i)).length()>0)
							{
								...
								if (((String) copyrights.get (i)).equals (NEW_COPYRIGHT))
								{
								...
								while (badId)
								{
									try
									{
										...
										if (((String) state.getAttribute(STATE_RESOURCES_MODE)).equalsIgnoreCase(RESOURCES_MODE_RESOURCES))
										{
											...
											boolean pubviewset = RealmService.getPubViewInheritance(ContentHostingService.getReference(resource.getId()));
											if (!pubviewset)
											{
												try
												{
													RealmService.setPubView(ContentHostingService.getReference(resource.getId()), ((Boolean) pubviews.get(i)).booleanValue());
												}
	...
	public void doAddfolder ( RunData data)
	{
		...
		{		
			...
			if (repeatedNames_index > 0)
			{
				...
				if (n.length ()>0)
				{
					addAlert(state,"Can not add folders with the same title " + n + ". ");
					unqualified_fields.set (repeatedNames_index, (String) unqualified_fields.get (repeatedNames_index) + n);
				}
			}
			else
					...
					if (state.getAttribute(STATE_MESSAGE) == null)
					{
						for (int i=1; i <= folderNumber; i++)
						{
							...
							try
							{
								...
								resourceProperties.addProperty (ResourceProperties.PROP_COPYRIGHT, (String)state.getAttribute (STATE_MY_COPYRIGHT));
								...
								if (((String) state.getAttribute(STATE_RESOURCES_MODE)).equalsIgnoreCase(RESOURCES_MODE_RESOURCES))
								{
									...
									boolean pubviewset = RealmService.getPubViewInheritance(ContentHostingService.getReference(collection.getId()));
									if (!pubviewset)
									{
										try
										{
											RealmService.setPubView(ContentHostingService.getReference(collection.getId()), ((Boolean) pubviews.get(i)).booleanValue());
										}
										catch (InUseException ee) {} 
									}
								}		
	...
	* @param data The RunData object for this reques
	*/
	public void doDelete ( RunData data)
	{
		...
		if (state.getAttribute(STATE_MESSAGE) == null)
		{
			...
			if (((String) state.getAttribute (STATE_SELECT_ALL_FLAG)).equals (BOOLEAN_TRUE_STRING))
			{
				state.setAttribute (STATE_SELECT_ALL_FLAG, BOOLEAN_FALSE_STRING);
			}
	...
	*/
	public void doHandlepaste ( RunData data)
	{
		...
		if (((String) state.getAttribute (STATE_CUT_FLAG)).equals (BOOLEAN_TRUE_STRING))
		{
			for (int i = 0; i < pasteCutItems.size (); i++)
			{
				...
				try
				{
					...
					if (properties.getProperty (ResourceProperties.PROP_IS_COLLECTION).equals (BOOLEAN_TRUE_STRING))
					{
					...
					else
					{
						...
						while ((!addResourceSuccess) && (!addResourceFailed))
						{
							ResourcePropertiesEdit resourceProperties = ContentHostingService.newResourceProperties ();
		...
		if (((String) state.getAttribute (STATE_COPY_FLAG)).equals (BOOLEAN_TRUE_STRING))
		{
		...
		if (state.getAttribute(STATE_MESSAGE) == null)
		{
			...
			if (((String)state.getAttribute (STATE_CUT_FLAG)).equals (BOOLEAN_TRUE_STRING))
			{
				state.setAttribute (STATE_CUT_FLAG, BOOLEAN_FALSE_STRING);
			}
			
			// reset the copy flag
			if (((String)state.getAttribute (STATE_COPY_FLAG)).equals (BOOLEAN_TRUE_STRING))
			{
				state.setAttribute (STATE_COPY_FLAG, BOOLEAN_FALSE_STRING);
			}
		}
	...
	public void doModifyproperties ( RunData data)
	{
		...
		if (state.getAttribute(STATE_MESSAGE) == null)
		{
			...
			try
			{
				...
				String newcopyright = params.getCleanString (NEW_COPYRIGHT);

				boolean pubviewset = false;
				boolean pubview = false;
				if (((String) state.getAttribute(STATE_RESOURCES_MODE)).equalsIgnoreCase(RESOURCES_MODE_RESOURCES))
				{
					pubviewset = RealmService.getPubViewInheritance(ContentHostingService.getReference(id));
					if (!pubviewset)
					{
				...
				if (!((pedit.getBooleanProperty (ResourceProperties.PROP_IS_COLLECTION)) && (id.equals ((String) state.getAttribute (STATE_HOME_COLLECTION_ID)))))
				{
					...
					{
						// title field is null
						addAlert(state,"The title field can not be null. ");
						u_fields = u_fields + ResourceProperties.PROP_DISPLAY_NAME;
				...
				if (!ContentHostingService.getProperties(id).getBooleanProperty(ResourceProperties.PROP_IS_COLLECTION))
				{
					...
					if (copyright!=null)
					{ 
						state.setAttribute(STATE_PROPERTIES_COPYRIGHT_CHOICE, copyright);
						...
						if (copyright.equals (NEW_COPYRIGHT))
						{
							...
							state.setAttribute(STATE_PROPERTIES_COPYRIGHT, newcopyright);
						}
						else
						{
							state.setAttribute(STATE_PROPERTIES_COPYRIGHT, mycopyright);
				...
				if (cedit != null)
				{
					...
					if (((String) state.getAttribute(STATE_RESOURCES_MODE)).equalsIgnoreCase(RESOURCES_MODE_RESOURCES))
					{
						...
						if (!pubviewset)
						{
							try
							{
								RealmService.setPubView(cedit.getReference(), pubview);
							}
							catch (InUseException ee) {} // %%%
						}
					}
				...
				else
				{
					...
					if (((String) state.getAttribute(STATE_RESOURCES_MODE)).equalsIgnoreCase(RESOURCES_MODE_RESOURCES))
					{
						...
						if (!pubviewset)
						{
							try
							{
								RealmService.setPubView(redit.getReference(), pubview);
							}
							catch (InUseException ee) {} // %%%
						}
					}
	...
	*/
	public void doReplacefile ( RunData data)
	{
		...
		if (repeatedNames_index >0)
		{
			...
			if (n.length ()>0)
			{
				addAlert(state,"Can not add replace file with the same title " + n + ". ");
				unqualified_fields.set (repeatedNames_index, (String) unqualified_fields.get (repeatedNames_index) + n);
			}
		}
		else
		{			
			...
			if (repeatedNames_index>0)
			{
				...
				{
					addAlert(state, "Can not find the resource which is to be replaced. ");
				}
			}	// if
		}	// if-else
	...
	*/
	private void doAdddocument ( RunData data, String contentType, String content)
	{
		...
		{
			// there is no input for the new copyright
			addAlert(state,"Please specify the copyright information. ");
			u_fields = u_fields + ResourceProperties.PROP_COPYRIGHT;
		}
		if (state.getAttribute(STATE_MESSAGE) == null)
		{
				...
				try
				{
					...
					if (((String) state.getAttribute(STATE_RESOURCES_MODE)).equalsIgnoreCase(RESOURCES_MODE_RESOURCES))
					{
		...
		
						{
							try
							{
								RealmService.setPubView(ContentHostingService.getReference(resource.getId()), (new Boolean(pubviewString)).booleanValue());
							}
							catch (InUseException ee) {} 
						}
					}
	...
	public void doAddurl ( RunData data )
	{
		...
		else
		{
			...
			for (int i=1; i<= UrlNumber; i++ )
			{
				...
				if ((copyright.equals (NEW_COPYRIGHT)) && (newcopyright.length ()==0))
				{
					// chosed to specify the copyright but not specifying the new copyright
					addAlert(state,"Please specify the new copyright information of URL item " + i + ". ");
					u_fields = u_fields + ResourceProperties.PROP_COPYRIGHT;
				}
			...
			if (state.getAttribute(STATE_MESSAGE) == null)
			{
				...
				if (repeatedNames_index > 0)
				{
					...
					if (n.length ()>0)
					{
						addAlert(state,"Can not add URL with the same title " + n + ". ");
						unqualified_fields.set (repeatedNames_index, (String) unqualified_fields.get (repeatedNames_index) + n);
					}
				}
				else
				{
					...
					else
					{
						for (int i=1; i<=UrlNumber; i++)
						{
							...
							if (((String) ((Vector) state.getAttribute (STATE_ADD_URL_COPYRIGHT)).get (i)).equals (NEW_COPYRIGHT))
							{
								resourceProperties.addProperty (ResourceProperties.PROP_COPYRIGHT, (String) ((Vector) state.getAttribute (STATE_ADD_URL_NEW_COPYRIGHT)).get (i));
							}
							else
							{
								resourceProperties.addProperty (ResourceProperties.PROP_COPYRIGHT, mycopyright);
	...
	*/
	public void doSort ( RunData data)
	{
		...
		else
		{
			...
			if (asc.equals (BOOLEAN_TRUE_STRING))
			{
				asc = BOOLEAN_FALSE_STRING;
			}
			else
			{
				asc = BOOLEAN_TRUE_STRING;
		...
		else
		{
			...
			for (int i=0; i<deleteIds.length; i++)
			{
				...
				try
				{
					...
					if (p.getPropertyFormatted (ResourceProperties.PROP_IS_COLLECTION).equals (BOOLEAN_TRUE_STRING))
					{
	...
	*/
	public void doPrereplace ( RunData data)
	{
		...
		{
			addAlert(state, " You can not replace more than " + MULTI_ADD_NUMBER + " items at one time. ");
		}
		else
		{
		...
		if (!qualify)
		{
			addAlert(state, " You can not replace folders, URLs, document files or shortcuts. To make changes, choose \"Revise\". ");
		}
	...
	public void doCut ( RunData data)
	{
		...
		else
		{
			...
			if (state.getAttribute(STATE_MESSAGE) == null)
			{
				...
				{
					state.setAttribute (STATE_CUT_FLAG, BOOLEAN_TRUE_STRING);
					if (((String) state.getAttribute (STATE_SELECT_ALL_FLAG)).equals (BOOLEAN_TRUE_STRING))
					{
						state.setAttribute (STATE_SELECT_ALL_FLAG, BOOLEAN_FALSE_STRING);
					}
	...
	*/
	public void doCopy ( RunData data )
	{
		...
		else
		{
			...
			if (state.getAttribute(STATE_MESSAGE) == null)
			{
				state.setAttribute (STATE_COPY_FLAG, BOOLEAN_TRUE_STRING);
				if (((String) state.getAttribute (STATE_SELECT_ALL_FLAG)).equals (BOOLEAN_TRUE_STRING))
				{
					state.setAttribute (STATE_SELECT_ALL_FLAG, BOOLEAN_FALSE_STRING);
				}
	...
	*/
	public void doSelectall ( RunData data)
	{
		...
		state.setAttribute (STATE_SELECT_ALL_FLAG,  BOOLEAN_TRUE_STRING);
...
//			Iterator allCollectionIterator = threadIterator(currentCollection.getMembers().iterator());
		...
		state.setAttribute (STATE_EXPAND_ALL_FLAG,  BOOLEAN_TRUE_STRING);
	}	// doExpandall
	...
	*/
									...
									boolean atHome)
	{
		...
		if (copyFlag.equals (BOOLEAN_TRUE_STRING) || cutFlag.equals (BOOLEAN_TRUE_STRING))
		{
			// has cut or copy
		...
		if (state.getAttribute (STATE_SELECT_ALL_FLAG).equals (BOOLEAN_FALSE_STRING))
		{
		...
		if (state.getAttribute (STATE_EXPAND_ALL_FLAG).equals (BOOLEAN_FALSE_STRING))
		{
	...
	*/
	protected void initState(SessionState state, VelocityPortlet portlet, JetspeedRunData data)
	{
		...
		state.setAttribute (STATE_SORT_ASC, BOOLEAN_TRUE_STRING);
		...
		state.setAttribute (STATE_INITIALIZED, BOOLEAN_TRUE_STRING);
				...
				if (displayName != null)
				{
					...
					for (int i=1; i <= testSize; i++)
					{
						...
						if ((testName != null) && (displayName.equals (testName)) 
						      &&  ((isCollection && collectionOrResource.equals (BOOLEAN_TRUE_STRING))
								        || (!isCollection && collectionOrResource.equals(BOOLEAN_FALSE_STRING))))
						{
							return i;
						}
					}	// for
...
//			String id = (String) collection.getId();
	...
	*/
	public void doFolder_permissions(RunData data, Context context)
	{
		...
		{
			addAlert(state, "You can not read this folder. ");
		}
		catch (IdUnusedException e)
		{
			addAlert(state, "Can not find the folder. ");
		}
