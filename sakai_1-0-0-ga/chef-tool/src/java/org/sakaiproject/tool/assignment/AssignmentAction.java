/**********************************************************************************
*
* $Header: /cvs/sakai/chef-tool/src/java/org/sakaiproject/tool/assignment/AssignmentAction.java,v 1.34 2004/10/07 21:12:24 zqian.umich.edu Exp $
*
***********************************************************************************
*
...
import org.sakaiproject.util.observer.MultipleEventsObservingCourier;
...
extends NewPagedResourceAction
{
	/** The attachments	 */
	...
	private static final String NEW_ASSIGNMENT_RELEASE_GRADE_TYPE = "new_assignment_release_grade_type";
	...
	private static final String MODE_STUDENT_LIST_ASSIGNMENTS = "Student View";
	...
	private static final String MODE_INSTRUCTOR_LIST_ASSIGNMENTS = "List of Assignments";
	...
	private static final String MODE_INSTRUCTOR_VIEW_STUDENTS_ASSIGNMENT = "List of Assignments by student";
	
	/** The instructor view of assignment submission report */
	private static final String MODE_INSTRUCTOR_REPORT_SUBMISSIONS = "Grade Report";
	...
	private static final String CLOSE_DUE_DATE_ERROR_MESSAGE = "Accept submissions deadline should be set after the due date. ";
	...
	{		
		...
		else if (mode.equals (MODE_STUDENT_VIEW_SUBMISSION))
		{
			...
			((MultipleEventsObservingCourier) state.getAttribute(STATE_OBSERVER)).disable();
	...
	protected String build_student_list_assignments_context (	VelocityPortlet portlet,
																...
																SessionState state)
	{		
		state.setAttribute(STATE_SELECTED_VIEW, "Student View");
		...
		Boolean a = new Boolean (AssignmentService.allowAddAssignment ((String) state.getAttribute (STATE_CONTEXT_STRING)));
		context.put ("allowAdd", a);
		
		// build the menu
		buildMenu (portlet, context, data, state, "studentForm", a.booleanValue (), false, false, "");
		
		context.put("allowAddNewAssignment", new Boolean(AssignmentService.allowAddAssignment ((String) state.getAttribute (STATE_CONTEXT_STRING))));

		add2ndToolbarFields(data, context);
		
		// inform the observing courier that we just updated the page...
		// if there are pending requests to do so they can be cleared
		((MultipleEventsObservingCourier) state.getAttribute(STATE_OBSERVER)).justDelivered();

		String template = (String) getContext(data).get("template");
		return template + TEMPLATE_STUDENT_LIST_ASSIGNMENTS;
		
	}	// build_student_list_assignmfents_context
															...
															SessionState state)
	{
		...
		{
			addAlert(state, "You are not allowed to view the submission to this assignment.");
		}
	...
	{		
		...
		{
			addAlert(state, "Can not find the assignment. ");
		}
		catch (PermissionException e)
		{
			addAlert(state, "You are not allowed to view the submission to this assignment. ");
														...
														SessionState state)
	{
		...
		{
			addAlert(state, "Can not find the assignment submission. ");
		}
		catch (PermissionException e)
		{
																...
																SessionState state)
	{
		...
		String gId = (String) state.getAttribute (STATE_CONTEXT_STRING);
		
		// filter out draft assignments from posted assignments
		Iterator assignmentsForContext = AssignmentService.getAssignmentsForContext (gId);
		Vector postedAssignments = new Vector ();
		Vector draftAssignments = new Vector ();
		Vector totalAssignments = new Vector ();
		while (assignmentsForContext.hasNext ())
		{
			Assignment a = (Assignment) assignmentsForContext.next ();
			if (a.getDraft())
			{
				draftAssignments.add(a);
			}
			else
			{
				postedAssignments.add(a);
			}
		}

		SortedIterator dIterator = new SortedIterator (draftAssignments.iterator(), new AssignmentComparator (sortedBy, sortedAsc));
		SortedIterator pIterator = new SortedIterator (postedAssignments.iterator(), new AssignmentComparator (sortedBy, sortedAsc));

		if (sortedAsc.equals(Boolean.TRUE.toString()))
		{
			// sort draft assignments first
			totalAssignments.addAll(iterator_to_vector(dIterator));
			totalAssignments.addAll(iterator_to_vector(pIterator));
		}
		else
		{
			// sort posted assignments first
			totalAssignments.addAll(iterator_to_vector(pIterator));
			totalAssignments.addAll(iterator_to_vector(dIterator));
		}
		
		//Iterator assignments = totalAssignments.iterator();
		Iterator assignments = prepPage(state).iterator();
		context.put ("sortedBy", sortedBy);
		context.put ("sortedAsc", sortedAsc);
		context.put ("assignments", assignments);
		...
		// build the menu
		buildMenu (portlet, context, data, state, "listAssignmentsForm", true, true, assignments.hasNext (), "");
																...
																SessionState state)
	{
		...
		context.put ("name_ReleaseGradeType", NEW_ASSIGNMENT_RELEASE_GRADE_TYPE);
		...
		context.put ("value_ReleaseGradeType", state.getAttribute (NEW_ASSIGNMENT_RELEASE_GRADE_TYPE));
	...
	{	
		...
		{
			addAlert(state, "You are not allowed to view the assignment. ");
		}
		...
		{
			addAlert(state, "Accept submissions deadline has not been specified. Please choose a date for it and post.");
		}
		context.put ("value_CloseMonth", state.getAttribute (NEW_ASSIGNMENT_CLOSEMONTH));
		context.put ("value_CloseDay", state.getAttribute (NEW_ASSIGNMENT_CLOSEDAY));
		context.put ("value_CloseYear", state.getAttribute (NEW_ASSIGNMENT_CLOSEYEAR));
		...
		for (int i=0; i<assignmentIds.size (); i++)
		{
			try
			{
				...
				if (submissions.hasNext ())
				{
					...
					addAlert(state, "Are you sure you want to delete " + a.getTitle () + " which has submissions already? ");
				}
				assignments.add (a);
			}
			catch (IdUnusedException e )
																...
																SessionState state)
	{
		...
		// build the menu
		buildMenu (portlet, context, data, state, "listSubmissionsForm", true, true, false, "listSubmission");
																		...
																		SessionState state)
	{
		...
		{
			assignmentsVector.add (assignments.next ());
		}
		
		List studentMembers = prepPage(state);
		...
		// build the menu
		buildMenu (portlet, context, data, state, "studentAssignmentForm", true, true, false, "studentAssignment");
	...
	{		
		...
		// build the menu
		buildMenu (portlet, context, data, state, "reportForm", true, true, false, "report");
	...
	String viewMode)
	{
		...
		if (instructorView)
		{
			if (viewMode.equalsIgnoreCase ("listSubmission"))
			{
				...
				bar.add ( new MenuEntry ("New...", null, true, MenuItem.CHECKED_NA, "doNew_assignment", formName) );
				bar.add ( new MenuEntry ("Release Grades", null, true, MenuItem.CHECKED_NA, "doRelease_grades", formName));
				...
				bar.add ( new MenuEntry ("Download All", "").setUrl(accessPointUrl));	
			}
			else if (viewMode.equalsIgnoreCase ("report"))
			{
				...
				if (assignments.hasNext())
				{
					...
					bar.add ( new MenuEntry ("Download Spreadsheet", "").setUrl(accessPointUrl));	
				}
			}
			...
			else
			{
				...
				bar.add ( new MenuEntry ("New...", null, true, MenuItem.CHECKED_NA, "doNew_assignment", formName) );
				bar.add ( new MenuEntry ("Revise", null, hasNextAssignment, MenuItem.CHECKED_NA, "doEdit_assignment", formName) );
				bar.add ( new MenuEntry ("Delete", null, hasNextAssignment, MenuItem.CHECKED_NA, "doDelete_confirm_assignment", formName) );
				bar.add ( new MenuEntry ("Duplicate", null,hasNextAssignment, MenuItem.CHECKED_NA, "doDuplicate_assignment", formName) );
				bar.add ( new MenuEntry ("Grade", null, hasNextAssignment, MenuItem.CHECKED_NA, "doGrade_assignment", formName) );
			}
	 ...
	 * @param context The context object
	 **/
	public void doPreview_submission (RunData data, Context context)
	{
		SessionState state = ((JetspeedRunData)data).getPortletSessionState (((JetspeedRunData)data).getJs_peid ());
	...
	public void doPreview_grade_submission (RunData data, Context context)
	{
		...
		{
			addAlert(state, "Please specify the grade. ");
		}
					...
					{			
						...
						if (state.getAttribute(STATE_MESSAGE) == null)
						{
							try
							{
								...
								{
									addAlert(state, "Grade can not be larger than the maximum grade. ");
								}
							}
							catch (NumberFormatException e){}
						}
	 ...
	 * @param context The context object
	 **/
	public void doDone_preview_submission (RunData data, Context context)
	{
		SessionState state = ((JetspeedRunData)data).getPortletSessionState (((JetspeedRunData)data).getJs_peid ());
	 ...
	 * @param context The context object
	 **/
	public void doCancel_show_submission (RunData data, Context context)
	{
		SessionState state = ((JetspeedRunData)data).getPortletSessionState (((JetspeedRunData)data).getJs_peid ());
	...
	private void grade_submission_option(RunData data, String gradeOption)
	{
		...
		{
			addAlert(state, INUSE_ERROR_MESSAGE + "submission. ");
		}	// try
	 ...
	 * @param context The context object
	 **/
	public void doSave_submission (RunData data, Context context)
	{
		SessionState state = ((JetspeedRunData)data).getPortletSessionState (((JetspeedRunData)data).getJs_peid ());
		ParameterParser params = data.getParameters ();
		...
		if (state.getAttribute(STATE_MESSAGE) == null)
		{
			try
			{
				...
				if (submission!=null)
				{
					...
					{
						addAlert(state, "Can not find the assignment " + a.getTitle ());
					}
					catch (PermissionException e)
					{
						addAlert(state, "You are not allowed to submit an answer. ");
					}
					catch (InUseException e)
					{
						addAlert(state, INUSE_ERROR_MESSAGE + "submission. ");
				...
				else
				{
					...
					{
						addAlert(state, "You are not allowed to create assignment submission. ");
					}
				}
			}
			catch (IdUnusedException e )
	 ...
	 * @param context The context object
	 **/
	public void doPost_submission (RunData data, Context context)
	{
		SessionState state = ((JetspeedRunData)data).getPortletSessionState (((JetspeedRunData)data).getJs_peid ());
		ParameterParser params = data.getParameters ();
		...
		if (state.getAttribute(STATE_MESSAGE) == null)
		{
			try
			{
				...
				if (a.getContent ().getHonorPledge ()!=1)
				{
					...
					{
						addAlert(state, "You have to check the honor pledge before submission. ");
					}
				...
				if (submissionType == 1)
				{
					...
					if (text.length ()==0)
					{
						addAlert(state, "You must type in your answer in the text input area before submission. ");
					}
				}
				else if (submissionType == 2)
				{
					...
					if ((v == null) || (v.size ()==0))
					{
						addAlert(state, "You must attach at least one document before submission. ");
					}
				}else if (submissionType==3)
				{
					...
					{
						addAlert(state, "You must either type in your answer in the text input or attach at least one document before submission. ");
					}
				}
			}
			catch (IdUnusedException e)
			...
			{
				addAlert(state, "You do not have permission to view the assignment. ");
			}
		}
		...
		if ((state.getAttribute(STATE_MESSAGE) == null) && (a != null))
		{
			try
			{
				...
				else
				{
					...
					{
						addAlert(state, "You are not allowed to submit an assignment. ");
					}
				}	// if -else
			}
			catch (IdUnusedException e )
	...
	protected void setNewAssignmentParameters(RunData data)
	{
		...
		if (title.length ()==0)
		{
			...
			addAlert(state, "Please specify the assignment title. ");
		}
		...
		int gradeType = Integer.parseInt (params.getString (NEW_ASSIGNMENT_GRADE_TYPE));
		...
		state.setAttribute (NEW_ASSIGNMENT_RELEASE_GRADE_TYPE, params.getString (NEW_ASSIGNMENT_RELEASE_GRADE_TYPE));
		...
		{
			addAlert(state, "Assignment due date should be set after the open date. ");
		}
		...
		{
			addAlert(state, "Assignment due date should not be set to be in the past. ");
		}
		...
		if (state.getAttribute(STATE_MESSAGE) == null)
		{
			if (((description == null) || (description.length() == 0)) && ((attachments == null || attachments.size() == 0)) && (state.getAttribute(NEW_ASSIGNMENT_DESCRIPTION_EMPTY) == null))
			{
				...
				addAlert(state, "This assignment has no instructions. Are you sure?");
	...
	{
		setNewAssignmentParameters(data);
		SessionState state = ((JetspeedRunData)data).getPortletSessionState (((JetspeedRunData)data).getJs_peid ());
	...
	public void doPost_draft_assignment (RunData data, Context context)
	{
		...
		try
		{
				...
				{				
					...
					try
					{
						CalendarEvent e = c.addEvent (	TimeService.newTimeRange(dueTime.getTime (), 0*60*1000),
														"Due: " + title,
														"Assignment " + title  + " is due on " + dueTime.toStringLocalFull () + ". ",
														"Deadline",
														/*location*/"",
														new ReferenceVector ());
					...
					{
						addAlert(state, "Can not add the due date to schedule. ");
					}
				} // if
			}	// if
			...
			if (checkAutoAnnounce.equalsIgnoreCase (Boolean.TRUE.toString ()) && (aEdit.getProperties ().getProperty (NEW_ASSIGNMENT_OPEN_DATE_ANNOUNCED)==null))
			{
				...
				{				
					...
					try
					{
						AnnouncementMessage message = channel.addAnnouncementMessage (	"Assignment: Open Date for "  + title,
																						false,
																						new ReferenceVector (),
																						"Open date for assignment " + FormattedText.convertPlaintextToFormattedText(title) + " is " + openTime.toStringLocalFull () + ". ");
		...
		{
			addAlert(state, "You are not allowed to post the drafted assignment. ");
		}
		catch (InUseException e)
		{
	...
	public void doPost_assignment (RunData data, Context context)
	{
		...
		{		
			...
			String releaseGradeType =(String) state.getAttribute (NEW_ASSIGNMENT_RELEASE_GRADE_TYPE);
			...
			if (state.getAttribute(STATE_MESSAGE) == null)
			{
				...
				if (assignmentContentId.length ()==0)
				{
					...
					else
					{
						...
						{
							addAlert(state, "You are not allowed to create an assignment content. ");
						}
					}
				}
				else
				{
					...
					{
						addAlert(state, "The assignment content is being edited by another user. ");
					}
					catch (IdUnusedException e )
					{
						addAlert(state, "Can not find the assignment content. ");
					}
					catch (PermissionException e)
					{
						addAlert(state, "You are not allowed to view the assignment content. ");
					}
				...
				if (assignmentId.length ()==0)
				{
					...
					{
						addAlert(state, "You are not allowed to add an assignment. ");
					}
				}
				else
				{
					...
					{
						addAlert(state, "The assignment is being edited by another user. ");
					}
					catch (IdUnusedException e )
					{
						addAlert(state, "Can not find the assignment. ");
				...
				if ((state.getAttribute(STATE_MESSAGE) == null) && (ac != null) && (a != null))
				{
					...
					// release type
					ac.setReleaseGrades (Boolean.valueOf (releaseGradeType).booleanValue ());
				...
				try
				{
						...
						if (c != null)
						{
							...
							try
							{
								...
								if (dueDateModified)
								{
									...
									else
									{
										...
										while ((!found) && (events.hasNext ()))
										{
											...
											if (((String) e.getDisplayName ()).indexOf ("Assignment " + title)!=-1)
											{
												found = true;
											}
										}
									...
									if (found)
									{
										...
										{
											addAlert(state, "Can not remove the old event for assignment " + title + ". ");
										}
										catch (InUseException ee)
										{
											addAlert(state, INUSE_ERROR_MESSAGE + "calendar. ");
										}
									}
								}
								e = null;
								e = c.addEvent (/*TimeRange*/ TimeService.newTimeRange(dueTime.getTime (), /*0 duration*/0*60*1000),
												/*title*/"Due: " + title,
												/*description*/ "Assignment " + title  + " is due on " + dueTime.toStringLocalFull () + ". ",
												/*type*/"Deadline",
												/*location*/"",
					...
					|| (openDateModified))
					{
						...
						if (channel != null)
						{
							...
							try
							{
								...
								if (!openDateModified)
								{
									header.setSubject(/*subject*/"Assignment: Open Date for " + title);
									message.setBody(/*body*/"Open date for assignment " + FormattedText.convertPlaintextToFormattedText(title)  + " is " + openTime.toStringLocalFull () + ". ");
								}
								else
								{
									header.setSubject(/*subject*/"Assignment: New Open Date for " + title);
									message.setBody(/*body*/"New open date for assignment " + FormattedText.convertPlaintextToFormattedText(title)  + " is " + openTime.toStringLocalFull () + ". ");
								}
					...
					else
					{
						...
						if (newAssignment)
						{
							...
							{
								addAlert(state, "You are not allowed to remove the assignment. ");
							}
	...
	public void doSave_assignment (RunData data, Context context)
	{
		...
		else
		{
			...
			{
				addAlert(state, INUSE_ERROR_MESSAGE + "content. ");
			}
		}
		...
		// release type
		cedit.setReleaseGrades (Boolean.valueOf (releaseGradeType).booleanValue ());
	...
	public void doEdit_assignment (RunData data, Context context)
	{
		...
		if (assignmentIds!=null)
		{
			...
			{
				addAlert(state, "Please choose only one assignment for revising.");
				editAllowed = false;
			}
			else
			{
				...
				String contextString = (String) state.getAttribute (STATE_CONTEXT_STRING);
				if (!AssignmentService.allowUpdateAssignment (assignmentIds[0]))
				{
					addAlert(state, "You are not allowed to modify the selected assignment.");
					editAllowed = false;
				}
				else
				{
					state.setAttribute (EDIT_ASSIGNMENT_ID, assignmentIds[0] );
					try
					{
						Assignment a = AssignmentService.getAssignment (assignmentIds[0]);
						
						Iterator submissions = AssignmentService.getSubmissions (a);
						if (submissions.hasNext ())
						{
							// if there is submission to the assignment, show the alert
							addAlert(state, "Assignment " + a.getTitle () + " has submissions.");
						}
						...
						{
							addAlert(state, "You are revising an assignment after the due date. ");
						}
						...
						state.setAttribute (NEW_ASSIGNMENT_RELEASE_GRADE_TYPE, String.valueOf (a.getContent ().releaseGrades ()));
		...
		{
			addAlert(state, "You must first choose an assignment by checking the box next to the assignment name. Then click the Revise button to revise the assignment.");
		}
	...
	public void doDelete_confirm_assignment (RunData data, Context context)
	{
		...
		if (assignmentIds!=null)
		{
			...
			for (int i=0; i<assignmentIds.length; i++)
			{
				...
				try
				{
					...
					{
						addAlert(state, "You are not allowed to remove " + "\"" + a.getTitle () + "\". ");
					}
				}
				catch (IdUnusedException e )
				{
		...
		{
			addAlert(state, "You must first choose the assignment(s) by checking the box next to the assignment name. Then click the Delete button to delete the assignment(s).");
		}
	...
	public void doDelete_assignment (RunData data, Context context)
	{
		...
		for (int i=0; i<ids.size (); i++)
		{
				...
				if (isThereEvent != null && isThereEvent.equals(Boolean.TRUE.toString()))
				{
					...
					if (c != null)
					{
						...
						if (oldEventId != null)
						{
							...
							{
								addAlert(state, "Can not find the calender event. ");
							}
							catch (PermissionException ee)
							{
							}
			...
			{
				addAlert(state, "You are not allowed to edit the assignment. ");
			}
		}	// for
	...
	public void doDeep_delete_assignment (RunData data, Context context)
	{
		...
		for (int i=0; i<ids.size (); i++)
		{
			...
			try
			{
				...
				{
					addAlert(state, "You are not allowed to remove assignment " + a.getTitle () + ". ");
				}
			}
			catch (IdUnusedException e )
			{
	...
	public void doDuplicate_assignment (RunData data, Context context)
	{
		...
		if (assignmentIds!=null)
		{
			for (int i=0; i<assignmentIds.length; i++)
			{
				try
				{
					AssignmentEdit aEdit = AssignmentService.addDuplicateAssignment (contextString, assignmentIds[i]);
				...
				{
					addAlert(state, "You are not allowed to duplicate assignment. ");
				}
				catch (IdInvalidException e)
				{
					addAlert(state, "The assignment id " + assignmentIds[i] + " is not valid. ");
				}
				catch (IdUnusedException e)
				{
					addAlert(state, "The assignment id " + assignmentIds[i] + " has not been used. ");
				}
				catch(Exception e)
				{}
		...
		{
			addAlert(state, "You must first choose an assignment by checking the box next to the assignment name. Then click the Duplicate button to duplicate the assignment. ");
		}
	...
	public void doGrade_assignment (RunData data, Context context)
	{
		...
		if (assignmentIds!=null)
		{
			if (assignmentIds.length >1)
			{
				// can not choose more than one assignment at one time
				addAlert(state, "Please choose only one assignment to grade.");
			}
			else
			{
				// get the assignment id
				String id = assignmentIds[0];
				state.setAttribute (EXPORT_ASSIGNMENT_REF, id );
				try
				{
					Assignment a = AssignmentService.getAssignment(id);
					state.setAttribute(EXPORT_ASSIGNMENT_ID, a.getId());
				...
				{
					addAlert(state, "There is no such assignment. ");
				}
				catch (PermissionException e)
				{
					addAlert(state, "You do not have permission to view the assignment. ");
		...
		{
			addAlert(state, "You must first choose an assignment by checking the box next to the assignment name. Then click the Grade button to grade the assignment.");
		}
	...
	public void doAttachments (RunData data, Context context)
	{
		...
		if (mode.equals (MODE_STUDENT_VIEW_SUBMISSION))
		{
			...
			state.setAttribute(AttachmentAction.STATE_FROM_TEXT, "the submission");
		}
		else if (mode.equals (MODE_INSTRUCTOR_NEW_ASSIGNMENT))
		{
			String stateFromText = "the new assignment";
			String title = (String) state.getAttribute (NEW_ASSIGNMENT_TITLE);
			if (title != null && title.length() > 0)
			{
				stateFromText = "new assignment " + '"' + title + '"';
			}
		...
		{
			String stateFromText = "the assignment";
			String title = (String) state.getAttribute (NEW_ASSIGNMENT_TITLE);
			if (title != null && title.length() > 0)
			{
				stateFromText = "assignment " + '"' + title + '"';
			}
			state.setAttribute(AttachmentAction.STATE_FROM_TEXT, stateFromText);
			setNewAssignmentParameters(data);
		}
	...
	*/
	protected void initState(SessionState state, VelocityPortlet portlet, JetspeedRunData data)
	{
		...
		{
			state.setAttribute(STATE_SELECTED_VIEW, "List of Assignments");
		}
		...
		if (cService == null)
		{
			...
			{
				calendarId = cService.
						calendarReference(PortalService.getCurrentSiteId(), SiteService.MAIN_CONTAINER);
				try
				{
		...
		if (aService == null)
		{
			...
			{
				channelId = aService.
						channelReference(PortalService.getCurrentSiteId(), SiteService.MAIN_CONTAINER);
				try
				{
		...
		{
			state.setAttribute (STATE_CONTEXT_STRING, PortalService.getCurrentSiteId());
		...
		updateObservation(state, portlet.getID());
	}	// initState
	
   /**
	* Setup our observer to be watching for change events
	* @param state The state object.
	*/
	private void updateObservation(SessionState state, String peid)
	{
		MultipleEventsObservingCourier observer = (MultipleEventsObservingCourier) state.getAttribute(STATE_OBSERVER);
	...
	private void resetAssignment (SessionState state)
	{
		...
		state.setAttribute (NEW_ASSIGNMENT_RELEASE_GRADE_TYPE, Boolean.FALSE.toString ());
		state.setAttribute (ResourceProperties.NEW_ASSIGNMENT_CHECK_ADD_DUE_DATE, Boolean.FALSE.toString ());
		state.setAttribute (ResourceProperties.NEW_ASSIGNMENT_CHECK_AUTO_ANNOUNCE, Boolean.FALSE.toString ());
	...
	private Hashtable monthTable ()
	{
		...
		n.put (new Integer (1), "JAN");
		n.put (new Integer (2), "FEB");
		n.put (new Integer (3), "MAR");
		n.put (new Integer (4), "APR");
		n.put (new Integer (5), "MAY");
		n.put (new Integer (6), "JUN");
		n.put (new Integer (7), "JUL");
		n.put (new Integer (8), "AUG");
		n.put (new Integer (9), "SEP");
		n.put (new Integer (10), "OCT");
		n.put (new Integer (11), "NOV");
		n.put (new Integer (12), "DEC");
		return n;
	...
	{
		Hashtable n = new Hashtable ();
		n.put (new Integer (2), "Letter grade");
		n.put (new Integer (3), "Points");
		n.put (new Integer (4), "Pass/Fail");
		n.put (new Integer (5), "Checkmark");
		n.put (new Integer (1), "Ungraded");
		return n;
	...
	{
		Hashtable n = new Hashtable ();
		n.put (new Integer (1), "Inline only");
		n.put (new Integer (2), "Attachments only");
		n.put (new Integer (3), "Inline and Attachments");
		return n;
	...
	private String getAssignmentSubmissionStatus (Assignment a, User u)
	{
			...
			if (submission!=null)
			{
				if (submission.getSubmitted ())
				{
					if (submission.getGraded () && submission.getGradeReleased ())
						...
						return "Returned " + submission.getTimeReturned ().toStringLocalFull ();
					else
						return "Submitted " +  submission.getTimeSubmitted ().toStringLocalFull ();
				}
				else
					return "Drafted";
			}
		...
		{
			return "Can not find the assignment submission. ";
		}
		catch (PermissionException e)
		{
			return "You are not allowed to view the submission to this assignment. ";
		}
	}
	
	/**
	 * Sort based on the given property
	 * @param data The RunData object for this request
	 */
			...
			if (submission !=null)
				if (submission.getSubmitted ())
					if (submission.getGraded () && submission.getGradeReleased ())
						...
						status = "Returned " + submission.getTimeReturned ().toStringLocalFull ();
					else
					{
						status = "Submitted" + submission.getTimeSubmitted ().toStringLocalFull ();
	...
	*/
	public void doPermissions(RunData data, Context context)
	{
		...
		state.setAttribute(PermissionsAction.STATE_DESCRIPTION, "Set permissions for Assignments in worksite "
	...
	*/
	protected List readAllResources(SessionState state)
	{
		...
		else if (mode.equalsIgnoreCase(MODE_STUDENT_LIST_ASSIGNMENTS))
		{
			...
			while (assignments.hasNext ())
			{
				...
				{
					addAlert(state, "Can not find assignment. ");
				}
				catch (PermissionException e)
				{
					addAlert(state, "You are not allowed to view the assignment. ");
	...
	public void doView (RunData data, Context context)
	{
		...
		if (viewMode.equalsIgnoreCase("List of Assignments"))
		{
			doList_assignments(data, context);
		}
		else if (viewMode.equalsIgnoreCase("List of Assignments by student"))
		{
			doView_students_assignment(data, context);
		}
		else if (viewMode.equalsIgnoreCase("Grade Report"))
		{
			doReport_submissions(data, context);
		}
		else if (viewMode.equalsIgnoreCase("Student View"))
		{
	...
	private void validPointGrade(SessionState state, String grade)
	{
		if (grade != null && !grade.equals(""))
		{
			if (grade.startsWith("-"))
			{
				...
				addAlert(state, "Please use zero or positive number for grade. ");
			}
			else
			{
				...
				if (index != -1)
				{
					...
					if (!grade.equals("."))
					{
						if (grade.length() > index + 2)
						{
							...
							addAlert(state, "Please use only one decimal place in grade. ");
						}
						else
						{
							...
							{
								addAlert(state, "Please use number for the grade field. ");
							}
						}
					}	
					else
