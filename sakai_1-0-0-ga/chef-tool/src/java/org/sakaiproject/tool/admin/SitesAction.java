/**********************************************************************************
*
* $Header: /cvs/sakai/chef-tool/src/java/org/sakaiproject/tool/admin/SitesAction.java,v 1.15 2004/09/30 20:20:56 ggolden.umich.edu Exp $
*
***********************************************************************************
*
	...
	protected List readAllResources(SessionState state)
	{
		...
		else
		{
			...
			for (Iterator iSites = sites.iterator(); iSites.hasNext();)
			{
				...
				if (	StringUtil.containsIgnoreCase(site.getId(),search)
					||	StringUtil.containsIgnoreCase(site.getTitle(),search)
					||	StringUtil.containsIgnoreCase(site.getDescription(),search)
	...
	*/
	private boolean readSiteForm(RunData data, SessionState state)
	{
		...
		if (site != null)
		{
			...
			catch (Exception e) { Log.warn("chef", this + ".readSiteForm(): bad status: " + status); }
