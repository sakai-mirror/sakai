/**********************************************************************************
*
* $Header: /cvs/sakai/chef-tool/src/java/org/sakaiproject/tool/admin/RealmsAction.java,v 1.12 2004/09/30 20:20:56 ggolden.umich.edu Exp $
*
***********************************************************************************
*
	...
	extends PagedResourceAction
{
	...
	protected List readAllResources(SessionState state)
	{
		...
		else
		{
			...
			for (Iterator iRealms = realms.iterator(); iRealms.hasNext();)
			{
				...
				if (	StringUtil.containsIgnoreCase(realm.getId(),search)
				/*	||	StringUtil.containsIgnoreCase(realm.getDescription(),search) TODO: removed for efficiency -ggolden */
					||	StringUtil.containsIgnoreCase(realm.getProviderRealmId(),search))
				{
					rv.add(realm);
				}
			}
	...
	*/
	private String buildEditContext(SessionState state, Context context)
	{
		...
		// get a list of auth locks
		List authLocks = new Vector();
		authLocks.addAll(realm.getAuthLocks());
		Collections.sort(authLocks);
		context.put("authLocks", authLocks);

		// get a list of auth roles
		List authRoles = new Vector();
		authRoles.addAll(realm.getAuthRoles());
		Collections.sort(authRoles);
		context.put("authRoles", authRoles);

		// get a list of anon locks
		List anonLocks = new Vector();
		anonLocks.addAll(realm.getAnonLocks());
		Collections.sort(anonLocks);
		context.put("anonLocks", anonLocks);

		// get a list of anon roles
		List anonRoles = new Vector();
		anonRoles.addAll(realm.getAnonRoles());
		Collections.sort(anonRoles);
		context.put("anonRoles", anonRoles);
	...
	*/
	private String buildEditUserContext(SessionState state, Context context)
	{
		...
		// get this user's abilities  (locks and roles)
	...
	*/
	private boolean readRoleForm(RunData data, SessionState state)
	{
		...
		if (role == null)
		{
			...
			// if the field is missing, and there are no locks or roles, just be done with no change
			if ((id == null) && (locks == null) && (roles == null)) return true;

			if (id == null)
			{
				addAlert(state, "Please enter a Role id.");
				return false;
				// %%% would be nice to read the roles and locks, and restore them when the form returns -ggolden
			}
		...
		// for each nested role set, give it to the role
		if (roles != null)
		{
			for (int i = 0; i < roles.length; i++)
			{
				Role nested = realm.getRole(roles[i]);
				if (!nested.equals(role)) role.addRole(nested);
			}
		}
	...
	*/
	private boolean readUserForm(RunData data, SessionState state)
	{
		...
		// for each lock set, give it to the user
		if (locks != null)
		{
			for (int i = 0; i < locks.length; i++)
			{
		...
		// for each role set, give it to the use
		if (roles != null)
		{
			for (int i = 0; i < roles.length; i++)
			{
