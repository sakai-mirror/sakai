/**********************************************************************************
*
* $Header: /cvs/sakai/util/src/java/org/sakaiproject/util/storage/BaseDbDoubleStorage.java,v 1.15 2004/10/14 23:00:23 janderse.umich.edu Exp $
*
***********************************************************************************
*
...
public class BaseDbDoubleStorage
{
	...
	*/
	 ...
	 */
	public List getResources(String channelRef, Time afterDate, int limitedToLatest, String draftsForId)
	{
		// if we are limiting, and are filtering out drafts, and don't have draft/owner support, filter here after
		boolean canLimit = true;
		boolean filterAfter = false;
		if (	(limitedToLatest > 0)
			&&	((m_resourceTableDraftField == null) || (m_resourceTableOwnerField == null))
			&&	(!"*".equals(draftsForId))
			)
		{
			canLimit = false;
		...
		// close the inner and add the limiter to the outer
		if ((limitedToLatest > 0) && canLimit)
		{
			if ("oracle".equals(m_sql.getVendor()))
			{
		...
		// after filter for draft and limit
		if (filterAfter)
		{
			Vector v = new Vector();
