/**********************************************************************************
*
* $Header: /cvs/sakai/framework-component/src/java/org/sakaiproject/component/framework/cluster/SakaiClusterService.java,v 1.5 2004/10/07 08:11:30 janderse.umich.edu Exp $
*
***********************************************************************************
*
		...
		public void run()
		{
			...
			while (!m_maintenanceCheckerStop)
			{
				try
				{
					...
					{
						statement = "select SERVER_ID from SAKAI_CLUSTER where SERVER_ID != ? and UPDATE_TIME < ("+sqlTimestamp()+" - "
					...
					{
						statement =
						"select SERVER_ID from SAKAI_CLUSTER where SERVER_ID != ? and UPDATE_TIME < "+sqlTimestamp()+" - INTERVAL "
						+ m_expired + " SECOND";
					}
