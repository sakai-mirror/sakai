 ...
 *         Comments
 */
public class ReviewBitMap
{
  private static org.apache.log4j.Logger LOG =
    org.apache.log4j.Logger.getLogger(ReviewBitMap.class);
  public BigInteger bigInteger;
  private int size;

  /**
   * Creates a new ReviewBitMap object.
   *
   * @param size DOCUMENTATION PENDING
   */
  public ReviewBitMap(int size)
  {
    if(LOG.isDebugEnabled())
    {
      LOG.debug("ReviewBitMap(int " + size + ")");
    }
  ...
  {
    LOG.debug("getReviewList()");
    ArrayList list = new ArrayList();
    for(int i = 0; i < size; i++)
    {
      boolean setted = this.bigInteger.testBit(i);
      if(setted)
      {
