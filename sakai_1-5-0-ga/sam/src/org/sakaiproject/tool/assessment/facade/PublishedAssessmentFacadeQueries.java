...
public class PublishedAssessmentFacadeQueries
  ...
  /**
     public static void main(String[] args) throws DataFacadeException {
       PublishedAssessmentFacadeQueries instance = new
      PublishedAssessmentFacadeQueries();
       if (args[0].equals("loadpublishA")) {
    PublishedAssessmentData p = instance.loadPublishedAssessment(new Long(
        args[1]));
    System.out.println("**print Published Assessment");
    printPublished(p);
       }
       if (args[0].equals("publishA")) {
    AssessmentData a = instance.loadAssessment(new Long(args[1]));
    System.out.println("**print Assessment");
    print(a);
    Long passessmentId = instance.publishAssessment(a);
    PublishedAssessmentData p = instance.loadPublishedAssessment(
        passessmentId);
    System.out.println("**print Published Assessment");
    printPublished(p);
    AuthorizationFacadeQueries instanceA = new AuthorizationFacadeQueries();
    // add Qualifier
    QualifierFacade q = new QualifierFacade(passessmentId.toString(),
                                            passessmentId.toString() + "_01",
        "a qualifier for a published assessment",
        null, "101");
    instanceA.addQualifier(q);
    // add authorization to these people
    AuthorizationFacade b = new AuthorizationFacade("2", "1",
        passessmentId.toString(), new Date(),
        null, "2", new Date(), new Boolean("true"));
    AuthorizationFacade c = new AuthorizationFacade("3", "1",
        passessmentId.toString(), new Date(),
        null, "2", new Date(), new Boolean("true"));
    AuthorizationFacade d = new AuthorizationFacade("4", "1",
        passessmentId.toString(), new Date(),
        null, "2", new Date(), new Boolean("true"));
    instanceA.addAuthz(b);
    instanceA.addAuthz(c);
    instanceA.addAuthz(d);
       }
       System.exit(0);
     }
   */
  ...
  public void createAuthorization(PublishedAssessmentData p) {
    ...
    // 3. create authorization for all the agentId in v
  ...
  public ArrayList getBasicInfoOfAllActivePublishedAssessments(String
    ...
    List l = getHibernateTemplate().find(
        "select new PublishedAssessmentData(p.publishedAssessmentId, p.title, c.releaseTo, c.startDate, c.dueDate, c.retractDate) " +
        " from PublishedAssessmentData p, PublishedAccessControl c " +
        " where c.assessment = p and p.status=1 " +
        " order by p." + orderBy);
  ...
  public ArrayList getBasicInfoOfAllInActivePublishedAssessments(String
    ...
    // this is a temporary sql stmt for getting the list, I shouldn't use SYSDATE
    // I will fix it after the Xmas break - daisyf
