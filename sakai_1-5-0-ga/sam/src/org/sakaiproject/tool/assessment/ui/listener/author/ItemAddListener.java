 ...
 * @version $Id: ItemAddListener.java,v 1.30.2.1 2005/02/16 21:41:09 lydial.stanford.edu Exp $
 */

public class ItemAddListener
    implements ActionListener {
  static Logger LOG = Logger.getLogger(ItemAddListener.class.getName());
...
// this is for fill in blank
      ...
      for (int i = 0; i < fibanswers.length; i++) {
        ...
        System.out.println(" lydiatest answers are = " + oneanswer);
        String[] multiplecorr = oneanswer.split("\\|");
        for (int j = 0; j < multiplecorr.length; j++) {
          System.out.println(" corr answers are for seq " + i + " " +
                             multiplecorr[j]);
          Answer answer1 = new Answer(text1, (String) multiplecorr[j],
  ...
  private String stripPtags(String origtext) {
   // interim solution for the wywisyg bug. This will strip off the first <p> and last </p> if both exists.  
    ...
    if ((origtext!= null)&& (origtext.startsWith("<p>")) && (origtext.endsWith("</p>")) ){
       newanswer = origtext.substring(3, origtext.lastIndexOf("</p>"));
