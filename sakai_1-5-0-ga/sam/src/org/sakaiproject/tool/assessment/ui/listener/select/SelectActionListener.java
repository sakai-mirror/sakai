 ...
 * @author Ed Smiley
 * @version $Id: SelectActionListener.java,v 1.37.2.5 2005/02/24 17:17:14 daisyf.stanford.edu Exp $
 */

public class SelectActionListener
    implements ActionListener {
  static Logger LOG = Logger.getLogger(SelectActionListener.class.getName());
  ...
  // 3. go through the pub list retrieved form DB and check if
  // agent is authorizaed and filter out the one that does not meet the
  // takeable criteria. The filtered one are possibly the submitted
  // assessment and we will put them in one pile too. This method results
      ...
      if (PersistenceService.getInstance().getAuthzQueriesFacade().isAuthorized(
        ...
        if (retractDate == null || retractDate.after(currentDate)) {
          if (dueDate == null || dueDate.after(currentDate) ||
              (dueDate.before(currentDate) && notSubmitted && acceptLateSubmission)) {
            if (totalSubmitted < maxSubmissionsAllowed) {
              if (startDate==null || (startDate!=null && startDate.before(currentDate)))
                takeableList.add(f);
            }
          }
        }
  ...
  private String hasFeedback(AssessmentGradingFacade a, HashMap publishedAssessmentHash){
    ...
    if ((AssessmentFeedbackIfc.IMMEDIATE_FEEDBACK).equals(p.getFeedbackDelivery())
        || ((AssessmentFeedbackIfc.FEEDBACK_BY_DATE).equals(p.getFeedbackDelivery())
             && p.getFeedbackDate()!= null && p.getFeedbackDate().before(currentDate))){
       hasFeedback="true";
     }
    return hasFeedback;
  }
