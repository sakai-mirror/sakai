 ...
 * $Id: ItemBean.java,v 1.18 2005/02/16 02:29:42 rgollub.stanford.edu Exp $
 */
public class ItemBean
  implements Serializable
{
  private static final org.apache.log4j.Logger LOG =
    org.apache.log4j.Logger.getLogger(ItemBean.class);
...
// end DELETEME
  ...
  public ItemBean()
  {
    LOG.debug("Constructor ItemBean()");
...
public boolean checkMatch(){
     ...
     if(choice==null ||choice.equals("")|| match==null || match.equals("")){
	   FacesContext context=FacesContext.getCurrentInstance();
	    FacesMessage message=new FacesMessage("You must enter Choice and Match values");
            context.addMessage("matchingF",message);
            return false;
     }
     else {
            return true;
...
//System.out.println("lydiatest and : " + apair.getIsCorrect().toString() + " " + apair.getCorrMatchFeedback()  + " " + apair.getIncorrMatchFeedback());
	...
	if(corrAnswer==null ||corrAnswer.equals("")){
	   FacesContext context=FacesContext.getCurrentInstance();
	    FacesMessage message=new FacesMessage("You must select one correct Answer");
            context.addMessage("addMcFail",message);
            return "MCAddFailure";
	}
        else {
            return "MCAddSuccess";
	...
	if(corrAnswer==null ||corrAnswer.equals("")){
	   FacesContext context=FacesContext.getCurrentInstance();
	    FacesMessage message=new FacesMessage("You must select one correct Answer");
            context.addMessage("poolAddMCFail",message);
             return "MCPoolAddFailure";
	}
        else {   
            return "MCPoolAddSuccess";
	}
    }
