...
* @version $Id: submissions.xsl,v 1.2 2004/08/03 18:23:44 lydial.stanford.edu Exp $
-->
...
<xsl:template name="submissions">
    ...
    <table align="{$tableAlign}" border="0" width="{$tableWidth}">
      ...
      <tr>
       ...
       <xsl:variable name="maxAttempts" select="/stxx/form/publishedAssessmentActionForm/maxAttempts"/>
      ...
      <tr>
        <td  valign="top" width="{$firstColWidth}"> <span class="form_label">Late Handling</span> <br/>
          ...
          <input disabled = "true" name="{$entryXpath}" type="radio" value="False">
            <xsl:if test="$lateHandling ='False'">
              <xsl:attribute name="checked">true</xsl:attribute>
            </xsl:if>
          </input> Late Submissions (After Due Date) will NOT be accepted<br/>
         
          <input  disabled = "true" name="{$entryXpath}" type="radio" value="True">
      ...
      <tr>
        <td  valign="top" width="{$firstColWidth}"> <span class="form_label">Auto Save</span> <br/>
          <xsl:variable name="autoSave" select="/stxx/questestinterop/assessment/qtimetadata/qtimetadatafield/fieldlabel[text()=
'AUTO_SAVE']/following-sibling::fieldentry"/>
          <xsl:variable name="entryXpath">
            <xsl:apply-templates select="/stxx/questestinterop/assessment/qtimetadata/qtimetadatafield">
