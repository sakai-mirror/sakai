      ...
      <tr>
        ...
        <xsl:variable name="xpathDuration" select="substring($temp,6)"/>
         ...
         <xsl:apply-templates select="/stxx/questestinterop/assessment/qtimetadata/qtimetadatafield">
          ...
          <input name="{$xpathDuration}" id="{$xpathDuration}" type="checkbox">
           <xsl:if test="$valueDuration='True'">
            <xsl:attribute name="checked">True</xsl:attribute>
            </xsl:if>
          <!--  <xsl:attribute name="onclick">javascript:disableFieldDependingOn("<xsl:value-of select="$xpathAutoSubmit"/>","<xsl:value-of select="$xpathDuration"/>",false);</xsl:attribute> -->
         ...
         <input name ="CONSIDER_DURATION" value = "{$durPosition}" type ="hidden" />
          ...
          <xsl:variable name="actionformHours">
            <xsl:value-of select="/stxx/form/assessmentActionForm/hours"/>
          </xsl:variable>
          <select name="hours">
            <option/>
          ...
          <!--****************Minutes***********************************-->
           &#160;: &#160;<xsl:variable name="actionformMin">
            <xsl:value-of select="/stxx/form/assessmentActionForm/minutes"/>
          </xsl:variable>
          <select name="minutes">
            <option/>
