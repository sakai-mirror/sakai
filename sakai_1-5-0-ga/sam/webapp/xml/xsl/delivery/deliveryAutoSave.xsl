 ...
 * @version $Id: deliveryAutoSave.xsl,v 1.1 2004/08/03 18:24:23 lydial.stanford.edu Exp $
-->
...
<xsl:import href="itemAutoSave.xsl" />
...
function autosave()
{
//    var asiurl = document.ASIDeliveryForm.action+"?autoSave=true";
//    top.dummyFrame.location = asiurl;

  document.ASIDeliveryForm.target = "dummyFrame";
  document.ASIDeliveryForm.submit();
//  document.ASIDeliveryForm.target = "_top";
}

function setTarget(loc)
{
  document.ASIDeliveryForm.target = loc;
}
</script>
...
-->
  <xsl:if test="stxx/form/xmlDeliveryActionForm/endTime != ''" >
    <xsl:choose>
      <xsl:when test="stxx/form/xmlDeliveryActionForm/duration != ''">
		  <div id="progressBar" >
		  <xsl:variable name="base"> <xsl:call-template name="baseHREF"/> </xsl:variable>
		  <script LANGUAGE="JavaScript" src="{$base}js/progressBar.js" />
		  <script LANGUAGE="JavaScript">
		  ...
		  <INPUT ID="progress_bar_btn" TYPE="button" VALUE="Hide Progress Bar" onclick="handleClick()" />
      </xsl:when>
      
    <xsl:otherwise>
		  <xsl:variable name="base"> <xsl:call-template name="baseHREF"/> </xsl:variable>
		  <script LANGUAGE="JavaScript" src="{$base}js/dueDate.js" />
		  <script LANGUAGE="JavaScript">
		    var begin = new Date("<xsl:value-of select="stxx/form/xmlDeliveryActionForm/beginTime"/> ");
		    var end = new Date("<xsl:value-of select="stxx/form/xmlDeliveryActionForm/endTime"/> ");
		    var now = new Date("<xsl:value-of select="stxx/form/xmlDeliveryActionForm/currentTime"/> ");
		  </script>	
		  <script LANGUAGE="JavaScript" src="{$base}js/dueDate2.js" />
    </xsl:otherwise>
  </xsl:choose>
  </xsl:if>
  </td>
...
<tr><td>&#160;</td><td align="right">
  ...
  <xsl:variable name="formname" select="concat($base,'asi/delivery/xmlAction.do')"/>
   <form action="{$formname}" target="_top" method="post" >
    <input>
...
<xsl:template match="stxx">
  ...
  <form action="{$formname}" method="post" name="ASIDeliveryForm">
        <input>
            ...
            <xsl:attribute name="name">autoSave</xsl:attribute>
...
<!--    <input type="submit" name="Save_and_Exit" value="Save and Exit" /> -->
  ...
  <input>
    ...
    <xsl:attribute name="onClick">setTarget("_self")</xsl:attribute>
  </input>
  <input>
    <xsl:attribute name="type">submit</xsl:attribute>
