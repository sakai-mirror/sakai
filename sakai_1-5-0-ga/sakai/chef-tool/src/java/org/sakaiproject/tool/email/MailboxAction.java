/**********************************************************************************
*
* $Header: /cvs/sakai/chef-tool/src/java/org/sakaiproject/tool/email/MailboxAction.java,v 1.14 2005/02/11 21:22:00 janderse.umich.edu Exp $
*
***********************************************************************************
*
...
// package
package org.sakaiproject.tool.email;
...
/**
* <p>MailboxAction is a the CHEF mailbox tool.</p>
*
	...
	extends PagedResourceAction
{
	...
	*/
		...
		implements Comparator
	{
		...
		* @return a a negative integer,  zero, or a positive integer
		...
		*/
		public int compare (Object o1, Object o2)
		{
			...
			switch (m_criteria)
			{
				...
				{
					return orderFactor *
						m1.getMailArchiveHeader().getDateSent().compareTo(
							m2.getMailArchiveHeader().getDateSent());
				}
	...
	*/
	protected List readAllResources(SessionState state)
	{
		...
		try
		{
			MailArchiveChannel channel = MailArchiveService.getMailArchiveChannel(
	...
	*/
	protected void initState(SessionState state, VelocityPortlet portlet, JetspeedRunData rundata)
	{
		...
		if (state.getAttribute(STATE_CHANNEL_REF) == null)
		{
			...
			if (channel == null)
			{
				channel = MailArchiveService.channelReference(PortalService.getCurrentSiteId(), SiteService.MAIN_CONTAINER);
			}
	...
	* @return (optional) template name for this panel
	*/
	private String buildViewModeContext(VelocityPortlet portlet, 
										Context context,
										RunData rundata,
										SessionState state)
	{
		...
		try
		{
			MailArchiveChannel channel = MailArchiveService.getMailArchiveChannel(
					(String) state.getAttribute(STATE_CHANNEL_REF));
			message = channel.getMailArchiveMessage((String) state.getAttribute(STATE_VIEW_ID));
		...
		if (allowDelete) bar.add( new MenuEntry("Remove", "doRemove"));
										...
										SessionState state)
	{
		...
		try
		{
			MailArchiveChannel channel = MailArchiveService.getMailArchiveChannel(
					(String) state.getAttribute(STATE_CHANNEL_REF));
			message =
				channel.getMailArchiveMessage((String) state.getAttribute(STATE_VIEW_ID));
			context.put("email", message);
		}
		catch (IdUnusedException e) {}
		catch (PermissionException e) {}
	...
	*/
	private String buildListModeContext(VelocityPortlet portlet, 
										Context context,
										RunData rundata,
										SessionState state)
	{
	...
	*/
	public void doRemove_confirmed(RunData data, Context context)
	{
		...
		try
		{
			...
			MessageEdit edit = channel.editMessage((String) state.getAttribute(STATE_VIEW_ID));
			channel.removeMessage(edit);
		}
		catch (PermissionException e)
		{
			addAlert(state, "You do not have permission to remove this message.\n");
		}
		catch (InUseException e)
		{
			addAlert(state, "Someone else is editing this message.\n");
		}
		catch (IdUnusedException e)
		{
			addAlert(state, "This message has already been removed.\n");
		}
	...
	*/
											...
											SessionState state)
	{
		...
		if (!all.isEmpty()) context.put("alias", all.get(0));
	...
	* update/save from the options process
	*/
	public void doUpdate(RunData data, Context context)
	{
		...
		{
			addAlert(state, "Cannot find the Email Archive Channel!\n");
		}
		...
		try
		{
			...
			if (size <= 0)
			{
				addAlert(state,"Page size must be a positive number.\n");
			}
		}
		catch (Exception any)
		{
		...
		if (alias != null)
		{
			...
			{
				addAlert(state, "The Email Alias must be made up of alpha numeric characters or any of !#$&'*+-=?^_`{|}~. (no spaces).\n");
			}
		}
		...
		{
			addAlert(state, "The Email Archive Channel may not be modified at this time.\n");
		}
		...
		if (state.getAttribute(STATE_MESSAGE) == null)
		{
			...
			if (StringUtil.different(curAlias, alias))
			{
				...
				if (ok)
				{
					...
					{
						addAlert(state, "The Email Alias cannot be changed at this time.\n");
					}
				}
				else
				{
					addAlert(state, "The Email Alias is otherwise in use.\n");
				}
			}
			...
			if (state.getAttribute(STATE_MESSAGE) == null)
			{
				...
				if (size != ((Integer) state.getAttribute(STATE_PAGESIZE)).intValue())
				{
					...
					tool.getPropertiesEdit().addProperty(PARAM_PAGESIZE, Integer.toString(size));
	...
	*/
	public void doPermissions(RunData data, Context context)
	{
		...
		state.setAttribute(PermissionsAction.STATE_DESCRIPTION, "Set permissions for Email Archive in worksite "
				+ SiteService.getSiteDisplay(channelRef.getContext()));

		// ... showing only locks that are prpefixed with this
		state.setAttribute(PermissionsAction.STATE_PREFIX, "mail.");
