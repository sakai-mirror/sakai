/**********************************************************************************
*
* $Header: /cvs/sakai/chef-tool/src/java/org/sakaiproject/tool/access/AccessServlet.java,v 1.23 2005/01/06 19:51:29 ggolden.umich.edu Exp $
*
***********************************************************************************
*
	...
	extends VmServlet
{
	...
	*/
	public void dispatch(HttpServletRequest req, HttpServletResponse res) throws ServletException
	{
		...
		if (path.endsWith("&copyrightAlert"))
		{
			copyrightAlert = true;
			path = path.substring(0, path.indexOf("&copyrightAlert"));
		}
		...
		try
		{
			...
			{
				respondError(req, res, ref.getReference(), "The resource is missing.");
			}
