...
/*import org.sakaiproject.service.syllabus.SyllabusEntry;
import org.sakaiproject.service.syllabus.SyllabusService;*/
...
/*			  HttpServletRequest req = (HttpServletRequest) FacesContext.getCurrentInstance()
			  ...
			  String siteId = ((ParameterParser) req.getAttribute("sakai.wrapper.params")).getString("site");*/
...
//			  syllabusItem = syllabusManager.getSyllabusItemByUserAndContextIds(userId, siteId);
			  ...
			  if(syllabusItem == null)
			  {
				  ...
				  {
				    syllabusItem = syllabusManager.createSyllabusItem(userId, siteId, null);
				  }
			  }
...
/*			  if((syllabusItem.getRedirectURL()!=null) && (!syllabusItem.getRedirectURL().equals("")))
			  {
			    ...
			    res.sendRedirect(syllabusItem.getRedirectURL());
					entries.clear();
					entry = null;
			  }
*/			  
				Set tempEntries = syllabusManager.getSyllabiForSyllabusItem(syllabusItem);
...
//      				    currentSiteId);
  	  syllabusItem = syllabusManager.getSyllabusItemByContextId(currentSiteId);
		  ...
		  if(syllabusItem == null)
		  {
			  ...
			  {
			    syllabusItem = syllabusManager.
		    			createSyllabusItem(currentUserId, currentSiteId, null);
			  }
		  }
		}
    catch(Exception e)
...
/*  public FileUpload getFileUpload() 
  ...
  public void setFileUpload(FileUpload fileUpload) 
  {
		  ...
		  else
		  {
		    ...
		    for (int i=0; i<selected.size(); i++)
		    {
		      ...
		      /*			  Iterator iter = dataSet.iterator();
		       ...
		       if(data.getSyllabusId().equals(den.getEntry().getSyllabusId()))
		       {
		       syllabusManager.removeSyllabusFromSyllabusItem(syllabusItem, data);
		       break;
		       }
		       }*/
		      syllabusManager.removeSyllabusFromSyllabusItem(syllabusItem, den.getEntry());
		    }
		  }
			entries.clear();
			entry = null;
		...
		try
		{
		  ...
		  else
		  {
		    ...
		    {	
		      ...
		      syllabusManager.addSyllabusToSyllabusItem(syllabusItem, getEntry().getEntry());
		    }
		  }
	...
	public String processListDelete()
		throws PermissionException
	{
		...
		try
		{
		  ...
		  else
		  {
		    if(entry.justCreated == false)
		    {
		      ...
		      /*			  Set dataSet = syllabusItem.getSyllabi();
		       Iterator iter = dataSet.iterator();
		       while(iter.hasNext())
		       {
		       SyllabusData data = (SyllabusData)iter.next();
		       ...
		       {
		       data.fakeUpdate(getEntry().getEntry());
		       }
		       }
		       syllabusItem.setSyllabi(dataSet);
		       syllabusManager.saveSyllabusItem(syllabusItem);*/
		    }
		  }	    
	    entries.clear();
	...
	public void downOnePlace(SyllabusData en)
	{
	  ...
	  Iterator iter = syllabusManager.getSyllabiForSyllabusItem(syllabusItem).iterator();
	  while(iter.hasNext())
	  {
	    SyllabusData data = (SyllabusData)iter.next();
	    if(en.equals(data))
	    {
	      if(iter.hasNext())
	        swapData = (SyllabusData)iter.next();
	      break;
	    }
	  }
...
/*
	  try
	  {
	    ...
	    HttpServletRequest currentRequest = (HttpServletRequest) exFc.getRequest();
	    
	    String[] fileNames ={filename};
	    
	    org.apache.commons.fileupload.FileUploadBase fu = new org.apache.commons.fileupload.DiskFileUpload();
	    
	    HttpServletRequest req = HttpServletRequestFactory.createValidHttpServletRequest(fileNames);
	    
	    java.util.List itemList = fu.parseRequest(req);
...
*/	  
	...
	public String processEditSaveRedirect()
		throws PermissionException
	{
...
//test wysiwyg for Lydia
/*  
  public void processTestWysiwyg(ValueChangeEvent event)
  {
    FacesContext context = FacesContext.getCurrentInstance();
    String type = (String) event.getNewValue();
  }
  public void processTestWysiwyg1()
  {
  }
*/
}