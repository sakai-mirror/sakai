...
import uk.ac.cam.caret.sakai.rwiki.model.RWikiObjectContent;
import uk.ac.cam.caret.sakai.rwiki.model.RWikiPermissions;
...
public abstract class RWikiObjectImpl implements RWikiObject
{
	 ...
	 */
	/* iebdelete public List getXHistory() {
		return m_history;
	}
	*/
	/**
	 ...
	 */
	/* iebdelete
	public void setXHistory(List history) {
...
//	}
	...
	public int compareTo(Object o)
	{
		...
		if (compare == 0)
		{
			...
			if ( compare == 0 )
			{
				compare = ( getRevision().compareTo(((RWikiObject)o).getRevision()) );
				if (compare == 0)
				{
					// compare serial number
					compare = getContent().compareTo(((RWikiObject) o).getContent());
				}
			}
			// if these are the same
		}
	...
	public String toString() {
		return this.getClass().toString() + " ID:" + this.getId() + " Name: " + this.getName();
	}
    ...
    public boolean updateContent(String content) {
        if (content.equals(this.m_content)) {
            return false;
        }
        // Copy current object to History.
        RWikiHistoryObject newHistoryObject = new RWikiHistoryObjectImpl();
        newHistoryObject.setContent(this.getContent());
    ...
    */
	...
	/*
	public RWikiHistoryObject getRevision(int revision) {
	    int numberOfRevisions = this.getNumberOfRevisions();
        
        if (revision >= 0
                && revision < numberOfRevisions) {
        	
        	// This needs to be a finder
            return ((RWikiHistoryObject) this.getHistory().get(revision));

        } else if (revision == numberOfRevisions) {
            RWikiHistoryObjectImpl mock = new RWikiHistoryObjectImpl();
            mock.setContent(this.getContent());
            mock.setRevision(numberOfRevisions);
            mock.setUser(this.getUser());
            mock.setVersion(this.getVersion());
            return mock;
        } else {
            throw new IllegalArgumentException(
                    "Invalid version number: " + revision);
        }   
    }
    */
    /*
    ...
    */
	...
	private void lazyLoadContentObject() {
		if ( codao == null ) {
//			Exception ex = new RuntimeException("TRACE: Content Object DAO is null");
//			System.err.println("Problem with loading Lazy Content, this is Ok, just means lazyLoadContent was called by Hibernate");
//			ex.printStackTrace();
			return;
		}
		if ( co == null ) {
	...
	public void setContent(String content) {
		...
		if ( co != null) // could be null if triggered during a hibernate template load
	...
	public String getContent() {
		...
		String content = null;
		if ( co != null ) content = co.getContent(); // could be null if triggerd during a template load
		if ( content == null ) content = "";
		return content;
	}
